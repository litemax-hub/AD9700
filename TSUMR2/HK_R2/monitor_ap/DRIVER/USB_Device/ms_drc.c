...
u8 OTG_Interrupt_Polling(u8 vec, USB_VAR *gUSBStruct)
{
		...
		while(1)
		{
			...
			else
			{
				if((((USB_REG_READ16(M_REG_DMA_MODE_CTL)) & 0x8000)==0) && (USB_REG_READ8(M_REG_DMA_MODE_CTL) !=0))
				{
					...
					USB_REG_WRITE16(0x8c,USB_REG_READ16(0x8c) & ~0x2000);
					//printf("A1:%x\n",USB_REG_READ16(0x8c*2));
					//printf("D1:%x\n",USB_REG_READ16(M_REG_DMA_MODE_CTL));
	...
	if (0x20 == vec)
	{
		...
		if  (intreg[0] & 0x0001)
		{
			...
			reg = USB_REG_READ8(0x102);//USB_REG_READ8(M_REG_CSR0);
			d.IIntSrc = EP_TX;
			d.IEP = 0;
	        d.IReg = reg;
	        d.ICause = IRC_EP;
	        ...
	        {
	            d.ICount = USB_REG_READ8(0x108);//USB_REG_READ16(M_REG_RXCOUNT);
	...
	{
		intreg[0] = USB_REG_READ16(M_REG_INTRTX);
		//printData("intreg:%x",USB_REG_READ16(M_REG_INTRTX));
		if(intreg[0] == 1)
		{
			return 0x21;
		}
		...
		while (intreg[0] & 0x2)
		{
	        if (intreg[0] & i)
	        {
	            ...
	            d.IReg = USB_REG_READ8(0x112);//USB_REG_READ8(M_REG_TXCSR1);
	            d.IIntSrc = EP_TX;
	            d.IEP = epnum;
...
void USB_Enable_EP_DRC(U8 ep,USB_VAR *gUSBStruct)
{
	...
	{
		USB_REG_WRITE16(0x124,0x400/*gUSBStruct->otgUSB_EP[ep].MaxEPSize*/);
		USB_REG_WRITE16(0x126,USB_REG_READ16(0x126) | 0x0080);
	}
	else
	{
		USB_REG_WRITE16(0x110,0x400/*gUSBStruct->otgUSB_EP[ep].MaxEPSize*/);
		USB_REG_WRITE16(0x112,0x0008 | 0x2000 | 0x0040);
	}
#if 0
...
void USB_Setup_Endpoint(U8 epnum,S8 epdir,S8 eptype,S8 epinf ,S16 epmaxps,USB_VAR *gUSBStruct)
{
...
void USB_DRC_Fifo_Write(U8 *srcP,U8 ep,USB_VAR *gUSBStruct)
{
    ...
    if(1)//((writecount > 0) && ((U32) srcP & 1))
    {
              while(writecount)
              {
              	  //printData("scrP:%x",*((U8 volatile *)srcP));
                  //*((U8 volatile *)fifo) = *((U8 volatile *)srcP);
                  msWriteByte((otgRegAddress + (ep<<2) + M_FIFO_EP0), *((U8 volatile *)srcP));
                  srcP++;
                  writecount--;
              }
    }
    else     /* word access if aligned */
    {
              while ((writecount >= 2) && !((U32) srcP & 1) )
              {
                  ...
                  msWrite2Byte((otgRegAddress + (ep<<2) + M_FIFO_EP0), *((U8 volatile *)srcP));
                  srcP = srcP + 2;
...
void USB_DRC_Fifo_Read(U8 *dstP,U8 ep,USB_VAR *gUSBStruct)
{
	...
	if(1)//((readcount > 0)&& ((U32) dstP & 1))
	{
		while (readcount > 0)
		{	
			*((U8 volatile *)dstP)= msReadByte((otgRegAddress + (ep<<2) + M_FIFO_EP0));//*((U8 volatile  *)fifo); //set address//wayne add
			//printData("dstp:%x", *((U8 volatile *)dstP));
	...
	else
	{
		...
		{
			*((U16 volatile *)dstP)= msRead2Byte((otgRegAddress + (ep<<2) + M_FIFO_EP0));//*((U16 volatile *)fifo); //set address//wayne add
...
U8 SDRAM2USB_Bulk(U32 txaddr,U32 txsize,USB_VAR *gUSBStruct)
{
	...
	inreg = USB_REG_READ8(0x112);//USB_REG_READ8(M_REG_TXCSR);
	...
	if  (outreg)
		USB_REG_WRITE8(0x112, outreg);//USB_REG_WRITE8(M_REG_TXCSR1, outreg);
...
void USB_DRC_Interrupt(USB_VAR *gUSBStruct)
{
	...
	if  (usbreg)
	{
		//printData("  INTRUSB:%x", USB_REG_READ16(M_REG_INTRUSB));
		//printf("M_REG_INTRUSB\n");
		d.IReg = usbreg;
		d.ICause = IRC_INTRUSB;
		USB_Parse_DRC_Int(&d,gUSBStruct);
	...
	if  (intreg[0] & 0x0001)
	{
        //USB_REG_WRITE8(M_REG_INDEX, 0);
        reg = USB_REG_READ8(0x102);//USB_REG_READ8(M_REG_CSR0);
		msWriteByte(otgRegAddress + 0x0e,0);
        ...
        {
            d.ICount = USB_REG_READ8(0x108);//USB_REG_READ16(M_REG_RXCOUNT);
            //printData("  ICount:%x", USB_REG_READ8(0x108));
			//printf("d.ICount:0x%x\n",d.ICount);
			//printf("d.IReg:0x%x\n",d.IReg);
            //if (d.ICount==0)
                //diag_printf("EP0RXCNT=0\n");
	...
	while (intreg[1])
	{
		...
		{
			//USB_REG_WRITE8(M_REG_INDEX, epnum);
			d.IReg = USB_REG_READ8(0x126);//USB_REG_READ8(M_REG_RXCSR1);
			d.IIntSrc = EP_RX;
			d.IEP = epnum;
			d.ICount = USB_REG_READ16(0x128);//USB_REG_READ16(M_REG_RXCOUNT);
			//printData( "EP_RX count:%x",d.ICount);
			//gprintf("EP_RX int\n");
			//printf("d.IReg:0x%x\n",d.IReg);
			//printf("d.ICount:0x%x\n",d.ICount);
			USB_Parse_DRC_Int(&d,gUSBStruct);
