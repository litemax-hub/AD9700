...
xdata DWORD _dramAddr = MIU_AUTOLOAD_ADDR_START;
xdata DWORD _dramXYVCCAddr = MIU_AUTOLOAD_ADDR_START + 0x2000;
#if XYVCC_ADL_DEGAMMA_600E
xdata DWORD _dramPostGammaAddr = MIU_AUTOLOAD_ADDR_START + 0x2000 + 0x4000;
#else
xdata DWORD _dramPostGammaAddr = MIU_AUTOLOAD_ADDR_START + 0x2000 + 0x3000;
#endif
...
void ClearHDRDramData(void)
{
	/*
    int idx = 0;
    //Occupied 8K
    mcu4kXdataMapToDRAMIMI(_dramAddr, 0);
    for(idx = 0; idx < X2D_MAP_SIZE; idx++)
    {
        _xdata2Dram[_xdataAddr + idx] = 0x00;
    }
    mcu4kXdataMapToDRAMIMI(_dramAddr + X2D_MAP_SIZE, 0);
    for(idx = 0; idx < X2D_MAP_SIZE; idx++)
    {
        _xdata2Dram[_xdataAddr + idx] = 0x00;
    }
...
void FireAutoDownload(BYTE u8WinIdx)
{
   HDR_PrintMsg("FireAutoDownload\n");
    ...
    DWORD u32ADLAddr =  (DEF_IMI_ADL_START_ADDR>>4);//0;//GetADLBaseAddr(u8WinIdx);
    //DWORD phyBaseAddr = _dramAddr & 0x7FFFFFFF;
    //DWORD u32Depth = AUTO_DOWNLOAD_HDR_TMO_SRAM_MAX_ADDR;

	msWrite2Byte( SC67_52, u32ADLAddr); //set autodownload address
	...
	msWriteByteMask( SC67_50,BIT1,BIT1); //auto download trigger
	...
	/*
    #if HDR_3DLUT
    u32Depth = AUTO_DOWNLOAD_HDR_3DLUT_SRAM_MAX_ADDR;
    #endif
    DWORD u32StartAddr = 0;
#if XVYCC_AUTODOWNLOAD
    FireXVYCCADL(_dramXYVCCAddr / BYTES_PER_WORD);
    FirePQGammaADL(_dramPostGammaAddr / BYTES_PER_WORD);
#endif
            //pnl mapping to src
    #if HDR_AUTODOWNLOAD_PATCH
    int idx = 0;
    DWORD addr1 = _xdataAddr + (64 + AUTO_DOWNLOAD_HDR_TMO_SRAM_MAX_ADDR) * BYTES_PER_WORD;
    u32Depth += AUTO_DOWNLOAD_HDR_TMO_SRAM_MAX_ADDR + 64;
    //memset 0 from #512 ~ #575
    #if HDR_3DLUT
    #else
    DWORD addr0 = _xdataAddr + AUTO_DOWNLOAD_HDR_TMO_SRAM_MAX_ADDR * BYTES_PER_WORD;
    memset((BYTE*)addr0, 0x00, 64 * BYTES_PER_WORD);
    #endif
    //memcpy 512 entries to #576
    memcpy((BYTE*)addr1, (BYTE*)(_xdataAddr), AUTO_DOWNLOAD_HDR_TMO_SRAM_MAX_ADDR *  BYTES_PER_WORD);

    for(idx = 0; idx < AUTO_DOWNLOAD_HDR_TMO_SRAM_MAX_ADDR; idx++)
    {
        WRITE_GAMMA_CTRL_FORMAT_1((BYTE*)(addr1 + idx * BYTES_PER_WORD), idx);
    }
    #endif

    //Dislabe ADL i64 Mode
    #if (ADL_I64MODE == 0)
    msWrite2Byte(u32BaseAddr + SC67_FE, 0x0002);
    #else
    msWrite2Byte(u32BaseAddr + SC67_FE, 0x0000);
    #endif
    // [15] disable auto download watch dog
    msWrite2ByteMask(u32BaseAddr + SC67_E0, 0x00, BIT15);

    // set baseaddr
    msWrite2Byte(u32BaseAddr + u32ADLAddr + SC67_52, ((phyBaseAddr + BYTES_PER_WORD * u32StartAddr) / BYTES_PER_WORD) & 0x0000FFFF);
    msWrite2ByteMask(u32BaseAddr + u32ADLAddr + SC67_54,  (((phyBaseAddr + BYTES_PER_WORD * u32StartAddr) / BYTES_PER_WORD) >> 16 & 0x0000FFFF), 0x07FF);

    // set depth
    msWrite2Byte(u32BaseAddr + u32ADLAddr + SC67_56, u32Depth);
    //set req len
    msWrite2Byte(u32BaseAddr + u32ADLAddr + SC67_58, 0x80);

    //__cpuc_flush_dcache_area(pu8AutoDownloadDRAMBaseAddr, u32Depth * BYTE_PER_WORD);
    // enable auto download
    msWriteByteMask(u32BaseAddr + u32ADLAddr + SC67_50, BIT0, BIT0);
...
void SetDeGammaToDiffDist(DWORD u32BaseAddr, BYTE IsEnDiffDist)
{
    if (IsEnDiffDist)
    {
        msWrite2Byte(u32BaseAddr + SC7C_84, 0xFFFF);
        msWrite2Byte(u32BaseAddr + SC7C_A0, 0x0008);
        msWrite2Byte(u32BaseAddr + SC7C_A2, 0x0201);
        msWrite2Byte(u32BaseAddr + SC7C_A4, 0x0303);
        msWrite2Byte(u32BaseAddr + SC7C_A6, 0x0403);
        msWrite2Byte(u32BaseAddr + SC7C_A8, 0x0404);
        msWrite2Byte(u32BaseAddr + SC7C_AA, 0x0404);
        msWrite2Byte(u32BaseAddr + SC7C_AC, 0x0504);
        msWrite2Byte(u32BaseAddr + SC7C_AE, 0x0605);
        msWrite2Byte(u32BaseAddr + SC7C_C0, 0);
        msWrite2Byte(u32BaseAddr + SC7C_C2, 2);
        msWrite2Byte(u32BaseAddr + SC7C_C4, 4);
        msWrite2Byte(u32BaseAddr + SC7C_C6, 8);
        msWrite2Byte(u32BaseAddr + SC7C_C8, 16);
        msWrite2Byte(u32BaseAddr + SC7C_CA, 32);
        msWrite2Byte(u32BaseAddr + SC7C_CC, 48);
        msWrite2Byte(u32BaseAddr + SC7C_CE, 64);
        msWrite2Byte(u32BaseAddr + SC7C_D0, 96);
        msWrite2Byte(u32BaseAddr + SC7C_D2, 128);
        msWrite2Byte(u32BaseAddr + SC7C_D4, 160);
        msWrite2Byte(u32BaseAddr + SC7C_D6, 192);
        msWrite2Byte(u32BaseAddr + SC7C_D8, 224);
        msWrite2Byte(u32BaseAddr + SC7C_DA, 256);
        msWrite2Byte(u32BaseAddr + SC7C_DC, 320);
        msWrite2Byte(u32BaseAddr + SC7C_DE, 384);
    }
    else
    {
        msWrite2Byte(u32BaseAddr + SC7C_84, 0xC000);
        msWrite2Byte(u32BaseAddr + SC7C_A0, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_A2, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_A4, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_A6, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_A8, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_AA, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_AC, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_AE, 0x070F);
        msWrite2Byte(u32BaseAddr + SC7C_C0, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_C2, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_C4, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_C6, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_C8, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_CA, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_CC, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_CE, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_D0, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_D2, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_D4, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_D6, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_D8, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_DA, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_DC, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_DE, 0x0100);
    }
}
...
void SetGammaToDiffDist(DWORD u32BaseAddr, BYTE IsEnDiffDist)
{
    if (IsEnDiffDist)
    {
        msWrite4Byte(u32BaseAddr + SC7C_04, 0xFFFFFFFF);
        msWrite2Byte(u32BaseAddr + SC7C_20, 0x0008);
        msWrite2Byte(u32BaseAddr + SC7C_22, 0x0201);
        msWrite2Byte(u32BaseAddr + SC7C_24, 0x0303);
        msWrite2Byte(u32BaseAddr + SC7C_26, 0x0303);
        msWrite2Byte(u32BaseAddr + SC7C_28, 0x0303);
        msWrite2Byte(u32BaseAddr + SC7C_2A, 0x0303);
        msWrite2Byte(u32BaseAddr + SC7C_2C, 0x0303);
        msWrite2Byte(u32BaseAddr + SC7C_2E, 0x0303);
        msWrite2Byte(u32BaseAddr + SC7C_30, 0x0303);
        msWrite2Byte(u32BaseAddr + SC7C_32, 0x0303);
        msWrite2Byte(u32BaseAddr + SC7C_34, 0x0303);
        msWrite2Byte(u32BaseAddr + SC7C_36, 0x0303);
        msWrite2Byte(u32BaseAddr + SC7C_38, 0x0303);
        msWrite2Byte(u32BaseAddr + SC7C_3A, 0x0303);
        msWrite2Byte(u32BaseAddr + SC7C_3C, 0x0403);
        msWrite2Byte(u32BaseAddr + SC7C_3E, 0x0404);

        msWrite2Byte(u32BaseAddr + SC7C_40, 0);
        msWrite2Byte(u32BaseAddr + SC7C_42, 2);
        msWrite2Byte(u32BaseAddr + SC7C_44, 4);
        msWrite2Byte(u32BaseAddr + SC7C_46, 8);
        msWrite2Byte(u32BaseAddr + SC7C_48, 16);
        msWrite2Byte(u32BaseAddr + SC7C_4A, 32);
        msWrite2Byte(u32BaseAddr + SC7C_4C, 48);
        msWrite2Byte(u32BaseAddr + SC7C_4E, 64);
        msWrite2Byte(u32BaseAddr + SC7C_50, 80);
        msWrite2Byte(u32BaseAddr + SC7C_52, 96);
        msWrite2Byte(u32BaseAddr + SC7C_54, 112);
        msWrite2Byte(u32BaseAddr + SC7C_56, 128);
        msWrite2Byte(u32BaseAddr + SC7C_58, 144);
        msWrite2Byte(u32BaseAddr + SC7C_5A, 160);
        msWrite2Byte(u32BaseAddr + SC7C_5C, 176);
        msWrite2Byte(u32BaseAddr + SC7C_5E, 192);
        msWrite2Byte(u32BaseAddr + SC7C_60, 208);
        msWrite2Byte(u32BaseAddr + SC7C_62, 224);
        msWrite2Byte(u32BaseAddr + SC7C_64, 240);
        msWrite2Byte(u32BaseAddr + SC7C_66, 256);
        msWrite2Byte(u32BaseAddr + SC7C_68, 272);
        msWrite2Byte(u32BaseAddr + SC7C_6A, 288);
        msWrite2Byte(u32BaseAddr + SC7C_6C, 304);
        msWrite2Byte(u32BaseAddr + SC7C_6E, 320);
        msWrite2Byte(u32BaseAddr + SC7C_70, 336);
        msWrite2Byte(u32BaseAddr + SC7C_72, 352);
        msWrite2Byte(u32BaseAddr + SC7C_74, 368);
        msWrite2Byte(u32BaseAddr + SC7C_76, 384);
        msWrite2Byte(u32BaseAddr + SC7C_78, 400);
        msWrite2Byte(u32BaseAddr + SC7C_7A, 416);
        msWrite2Byte(u32BaseAddr + SC7C_7C, 448);
        msWrite2Byte(u32BaseAddr + SC7C_7E, 480);
    }
    else
    {
        msWrite4Byte(u32BaseAddr + SC7C_04, 0xFF800000);
        msWrite2Byte(u32BaseAddr + SC7C_20, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_22, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_24, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_26, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_28, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_2A, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_2C, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_2E, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_30, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_32, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_34, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_36, 0x0800);
        msWrite2Byte(u32BaseAddr + SC7C_38, 0x0100);
        msWrite2Byte(u32BaseAddr + SC7C_3A, 0x0302);
        msWrite2Byte(u32BaseAddr + SC7C_3C, 0x0504);
        msWrite2Byte(u32BaseAddr + SC7C_3E, 0x0706);

        msWrite2Byte(u32BaseAddr + SC7C_40, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_42, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_44, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_46, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_48, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_4A, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_4C, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_4E, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_50, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_52, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_54, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_56, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_58, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_5A, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_5C, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_5E, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_60, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_62, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_64, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_66, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_68, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_6A, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_6C, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_6E, 0x0000);
        msWrite2Byte(u32BaseAddr + SC7C_70, 0x0002);
        msWrite2Byte(u32BaseAddr + SC7C_72, 0x0004);
        msWrite2Byte(u32BaseAddr + SC7C_74, 0x0008);
        msWrite2Byte(u32BaseAddr + SC7C_76, 0x0010);
        msWrite2Byte(u32BaseAddr + SC7C_78, 0x0020);
        msWrite2Byte(u32BaseAddr + SC7C_7A, 0x0040);
        msWrite2Byte(u32BaseAddr + SC7C_7C, 0x0080);
        msWrite2Byte(u32BaseAddr + SC7C_7E, 0x0100);
    }
}
#if ENABLE_LUT_AUTODOWNLOAD

extern code BYTE* tAllFrontEndGammaTab[] ;
extern code BYTE* tAllxvYccFixGammaTab[] ;

extern code BYTE tblFixGamma22_N[];
extern code BYTE tblFixGamma22_N_EXT[];
extern code BYTE tAdobeDeGamma[];//Gamma 2.2
void WriteXVYCCTable(BYTE u8WinIdx)
{
	...
	//IMI_Init(FIXGAMMA_ADL_ADDR, FIXGAMMA_ADL_SIZE);
        ...
        {
            pu8Tab1 = (BYTE*)((void*)tAdobeDeGamma);
        ...
        for (u8TgtChannel=0; u8TgtChannel<3; u8TgtChannel++)
        {
            ...
            #if XVYCC_GAMMA_EXT	
	    	pu8Tab1 = (BYTE*)((void*)tblFixGamma22_N_EXT);
...
/*
    xdata DWORD idx = 0;
    xdata DWORD val= 0;
    xdata DWORD address = _dramXYVCCAddr;

    #if XYVCC_ADL_DEGAMMA_600E
    //de gamma
    mcu4kXdataMapToDRAMIMI(_dramXYVCCAddr, 0);
    address = _xdataAddr;
    for(idx = 0; idx < 256; idx++)
    {
        val = _SRGBDeGamma[idx * 2 + 1];
        val <<= 8;
        val += _SRGBDeGamma[idx * 2];
        WriteXVYCCDegamma(address, val);
        address += BYTES_PER_WORD;
    }

    mcu4kXdataMapToDRAMIMI(_dramXYVCCAddr + X2D_MAP_SIZE, 0);
    address = _xdataAddr;
    for(idx = 0; idx < 256; idx++)
    {
        WriteXVYCCDegamma(address, val);
    ...
    mcu4kXdataMapToDRAMIMI(_dramXYVCCAddr + X2D_MAP_SIZE*2, 0);
    address = _xdataAddr;
    for(idx = 0; idx < 256; idx++)
    {
        if (idx < 88)
    ...
    //fix gamma
    mcu4kXdataMapToDRAMIMI(_dramXYVCCAddr + X2D_MAP_SIZE*3, 0);
    address = _xdataAddr;
    for(idx = 0; idx < 128; idx++)
    {
        val = _SRGBFixGamma[idx * 3 + 2] & 0x0F;
        val <<= 8;
        val += _SRGBFixGamma[idx * 3];
        WriteXVYCCGamma(address, val);
        address += BYTES_PER_WORD;

        val = _SRGBFixGamma[idx * 3 + 2] & 0xF0;
        val <<= 4;
        val += _SRGBFixGamma[idx * 3 + 1];
        WriteXVYCCGamma(address, val);
        address += BYTES_PER_WORD;
    }
    #else
...
void WritePostGammaTable(BYTE u8WinIdx)
{
    ...
    XDATA BYTE idx, j;
    XDATA BYTE NvrValue;
    XDATA BYTE ChannelIndex;
    XDATA WORD DiffValue0;
    XDATA WORD DiffValue1;
    XDATA WORD GammaTbl[4];
    XDATA DWORD address;

    //post gamma
    mcu4kXdataMapToDRAMIMI(_dramPostGammaAddr,0);
    ...
    for(ChannelIndex = 0; ChannelIndex < 3; ChannelIndex++)
	{
        ...
        for(idx = 0; idx < ACHDRGammaTblSize; idx++)
        {
            //Read gamma table
            if(!useDefaultTbl)
            {
                #if USEFLASH
                NvrValue = FlashReadByte(ACHDRGammaTblAddr + ACHDRGammaTblSize*ChannelIndex + idx);
                #else
                NVRam_ReadByte(ACHDRGammaTblAddr + ACHDRGammaTblSize*ChannelIndex + idx, &NvrValue);
                #endif
            }
            ...
            if(useDefaultTbl)
            {
                DiffValue1 = _DefaultPostGamma[idx] & 0x0F;
                DiffValue0 = (_DefaultPostGamma[idx] & 0xF0) >> 4;
            }
            else
            {
            ...
            GammaTbl[0] = (idx == 0)? 0 : (GammaTbl[3] + DiffValue0);
            GammaTbl[1] = (idx == 0)? (DiffValue0*2) : (GammaTbl[0] + DiffValue0);
            ...
            for(j=0; j<4; j++)
            {
                ...
                WriteVOPPostGamma(address, (GammaTbl[j]<<2), ChannelIndex);
                address += BYTES_PER_WORD;
            }
        }
    }
    #else
    WriteVOPPostGamma(_dramPostGammaAddr, 0xFF, 0);
    #endif
    UNUSED(u8WinIdx);
}
#endif
...
xdata MS_U32 i = 0;
    ...
    switch((int)type)
    {
        case HDR_DeGammaTbl:
        {
            ...
            //}
            IMIAddress = DEF_IMI_ADL_START_ADDR;//0;
	    ...
	    IMIAddress = DEF_IMI_ADL_START_ADDR+AUTO_DOWNLOAD_HDR_TMO_SRAM_MAX_ADDR*BytesPerEntryDe;
        }break;

		 case HDR_ToneMappingTbl:
        {
            SetGammaToDiffDist(u32BaseAddr, IsDiffDist);
            IMIAddress = DEF_IMI_ADL_START_ADDR+AUTO_DOWNLOAD_HDR_TMO_SRAM_MAX_ADDR*BytesPerEntryGa+AUTO_DOWNLOAD_HDR_TMO_SRAM_MAX_ADDR*BytesPerEntryDe;
        }break;
    }
    ...
    if (type == HDR_3DLutTbl)
    {
       ...
       BYTE header[32] = {0xF7,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
		for(i=0;i<32;++i)
		IMI_WriteByte(DEF_IMI_ADL_START_ADDR+i, header[i]&0xFF);
...
void ConfigB01_Y2R(BYTE u8WinIdx)
{
    ...
    //Y2R round
    msWriteByteMask(u32BaseAddr + SC7B_0E, BIT1, BIT1);
    //Y2R shift
    msWriteByteMask(u32BaseAddr + SC7B_0F, BIT0, BIT0|BIT1);
    ...
    //3x3 coefficents
    msWrite2Byte(u32BaseAddr + SC7B_10, coeff[1]);
    msWrite2Byte(u32BaseAddr + SC7B_12, coeff[2]);
    msWrite2Byte(u32BaseAddr + SC7B_14, coeff[0]);
    msWrite2Byte(u32BaseAddr + SC7B_16, coeff[4]);
    msWrite2Byte(u32BaseAddr + SC7B_18, coeff[5]);
    msWrite2Byte(u32BaseAddr + SC7B_1A, coeff[3]);
    msWrite2Byte(u32BaseAddr + SC7B_1C, coeff[7]);
    msWrite2Byte(u32BaseAddr + SC7B_1E, coeff[8]);
    msWrite2Byte(u32BaseAddr + SC7B_20, coeff[6]);

    //YCbCr offset
    msWrite4Byte(u32BaseAddr + SC7B_22, pYCbCrOffset[0]);
    msWrite4Byte(u32BaseAddr + SC7B_26, pYCbCrOffset[1]);
    msWrite4Byte(u32BaseAddr + SC7B_2A, pYCbCrOffset[2]);

    //range min
    msWrite2Byte(u32BaseAddr + SC7B_2E, 0x0000);
     //range max
    msWrite2Byte(u32BaseAddr + SC7B_30, 0xFFFF);
    //inv
    msWrite4Byte(u32BaseAddr + SC7B_34, 0x00001);
    //force disable IP2 CSC
    msWrite2Byte(u32BaseAddr + SC22_56, 0x0000); 

	 //overscan with Green line
    if (cf.ucColorType  == COMBO_COLOR_FORMAT_RGB)
    {
    	msWriteByteMask(u32BaseAddr + SC79_06, BIT7, BIT7);
    }
    else
    {
    	msWriteByteMask(u32BaseAddr + SC79_06, 0x00, BIT7);
    }
...
void ConfigB01_CSC(BYTE u8WinIdx)
{
    ...
    msWriteBit(u32BaseAddr + SC7B_0F, FALSE, BIT2);
    msWriteBit(u32BaseAddr + SC7B_0F, TRUE, BIT3);
    for(idx = 0; idx < 9 ; idx++)
    {
        msWrite2Byte(u32BaseAddr + SC7B_40 + idx * 2, byp3x3[idx]);
    }
}
...
void ConfigB01_R2Y(BYTE u8WinIdx)
{
    ...
    //R2Y round
    msWriteByteMask(u32BaseAddr + SC7B_0E, BIT3, BIT3);
    //R2Y shift
    msWriteByteMask(u32BaseAddr + SC7B_0F, BIT4, BIT4|BIT5);

    for(idx = 0; idx < 9 ; idx++)
    {
        msWrite2Byte(u32BaseAddr + SC7B_52 + idx * 2, p3x3[idx]);
    }
    //Y/Cb/Cr offset
    msWrite2Byte(u32BaseAddr + SC7B_64, 0x0000);
    msWrite2Byte(u32BaseAddr + SC7B_66, 0x4000);
    msWrite2Byte(u32BaseAddr + SC7B_68, 0x4000);

    //clip min
    msWrite2Byte(u32BaseAddr + SC7B_6A, 0x0000);
    msWrite2Byte(u32BaseAddr + SC7B_6C, 0x8001);
    msWrite2Byte(u32BaseAddr + SC7B_6E, 0x8001);

     //clip max
    msWrite2Byte(u32BaseAddr + SC7B_70, 0x7FFF);
    msWrite2Byte(u32BaseAddr + SC7B_72, 0x7FFF);
    msWrite2Byte(u32BaseAddr + SC7B_74, 0x7FFF);
    ...
    if(IsEnable)
    {
	  ...
	  { //YCbCr
        ...
        if (u8WinIdx == 0)
        {
            for(idx = 0; idx < 9 ; idx++)
            {
                msWrite2ByteMask(SC10_4C + idx * 2, p3x3[idx], 0x1FFF);
            }
            //Main Window Matrix Enable
            msWriteByte(SC10_5E, 0x35); //Full ragnge Y2R
            msWriteByteMask(SC0F_AE,0 ,BIT6); //Y sub 16 disable

            HDR_PrintMsg("Main Window Matrix Enable Y2R");
        }else{
            for(idx = 0; idx < 9 ; idx++)
            {
                msWrite2Byte(SC0F_3A + idx * 2, p3x3[idx]);
            }
	    HDR_PrintMsg("Sub Window Matrix Enable Y2R");
            //Sub Window Matrix Enable
            msWriteByte(SC0F_4C, 0x35);//Full ragnge Y2R
            msWriteByteMask(SC0F_AE,0 ,BIT7); //Y sub 16 disable
        }
#endif
...
void EnableHDRIP(BYTE u8WinIdx, BOOL IsOn)
{
    ...
    if(TRUE == IsOn)
    {
        msWriteByteMask(u32BaseAddr + SC79_0D, 0, BIT7);
        //ip2 path en
        msWriteByteMask(u32BaseAddr + SC79_0E, BIT1, BIT1);

        //Greg : HDMI driver do 422 to 444 automatically, so no need to do 422
        // to 444 in HDR IP.
	 msWriteByteMask(u32BaseAddr + SC79_0E, BIT4|BIT5, BIT4|BIT5);
        msWriteByteMask(u32BaseAddr + SC7A_03, 0x00, BIT7);
    }
    else
    {
        msWriteByteMask(u32BaseAddr + SC79_0D, BIT7, BIT7);
        //ip2 path en
        msWriteByte(u32BaseAddr + SC79_0E, 0x00);
    }
    //hdr open standard en
    msWriteByteMask(u32BaseAddr + SC79_0F, BIT7, BIT7);
    msWriteByteMask(u32BaseAddr + SC79_07, 0x00, BIT6|BIT7);
    msWriteByteMask(u32BaseAddr + SC79_09, BIT2|BIT0, BIT2|BIT0);
}
...
{
    DWORD u32BaseAddr = GetHDRBaseAddr(u8WinIdx);
    msWrite2Byte(u32BaseAddr + SC7B_7E, adjChromaWeight);
}
...
void ConfigB02_YFullToRFull(BYTE u8WinIdx)
{
    ...
    msWrite2Byte(u32BaseAddr + SC7A_E8, reg_b02_y2r_coef02);
    msWrite2Byte(u32BaseAddr + SC7A_EA, reg_b02_y2r_coef11);
    msWrite2Byte(u32BaseAddr + SC7A_EC, reg_b02_y2r_coef12);
    msWrite2Byte(u32BaseAddr + SC7A_EE, reg_b02_y2r_coef21);
    msWrite4Byte(u32BaseAddr + SC7A_F0, reg_b02_y2r_off0);
    msWrite4Byte(u32BaseAddr + SC7A_F4, reg_b02_y2r_off1);
    msWrite4Byte(u32BaseAddr + SC7A_F8, reg_b02_y2r_off2);
}

void ConfigB04(BYTE u8WinIdx)
{
    DWORD u32BaseAddr = GetHDRBaseAddr(u8WinIdx);
    msWrite2Byte(u32BaseAddr + SC7A_80, 0x0000);
    msWrite2Byte(u32BaseAddr + SC7A_82, 0x07FF);
    msWrite2Byte(u32BaseAddr + SC7A_84, _satGain);
    msWrite4Byte(u32BaseAddr + SC7A_86, _hk_high_th);
    msWrite4Byte(u32BaseAddr + SC7A_8A, 0x00000000);
    msWrite4Byte(u32BaseAddr + SC7A_90, 0x00000000);
    msWrite4Byte(u32BaseAddr + SC7A_94, 0xB800);
    msWrite4Byte(u32BaseAddr + SC7A_96, 0x0000);
}
...
void _hdr3dLutSettings_b502W(BYTE u8WinIdx,  BYTE bIsOn, WORD u16iMaxC, WORD u16iMinC, DWORD u32iDistiCInv, WORD u16iMinY, WORD u16iMaxY, DWORD u32iDistiYInv, BYTE bAlphaRound, BYTE u8OutControl, BYTE bOutControlRound, BYTE bLutBlendMode, BYTE b3dLutDitherMode)
{
    ...
    if(0 == bIsOn)
    {
        msWriteByteMask(u32BaseAddr + SC7A_04, 0x10, 0x10);
    }
    else
    {
        #if HDR_3DLUT
        msWriteByteMask(u32BaseAddr + SC7A_04,0x00,0x10); //reg_b06_byp_en
        #else
        msWriteByteMask(u32BaseAddr + SC7A_04,0x10,0x10); //reg_b06_byp_en
        #endif
    }
    msWrite2Byte(u32BaseAddr + SC7A_A0,u16iMaxC);
    msWrite2Byte(u32BaseAddr + SC7A_A2,u16iMinC);
    msWrite4Byte(u32BaseAddr + SC7A_A4,u32iDistiCInv);
    msWrite2Byte(u32BaseAddr + SC7A_A8,u16iMinY);
    msWrite2Byte(u32BaseAddr + SC7A_AA,u16iMaxY);
    msWrite4Byte(u32BaseAddr + SC7A_AC,u32iDistiYInv);
    msWriteByteMask(u32BaseAddr + SC7A_A0,0x01,bAlphaRound);
    msWriteByteMask(u32BaseAddr + SC7A_A0,0x06,u8OutControl<<1);
    msWriteByteMask(u32BaseAddr + SC7A_A0,0x08,bOutControlRound<<3);
    msWriteByteMask(u32BaseAddr + SC7A_A0,0x08,bLutBlendMode<<3);
    msWriteByteMask(u32BaseAddr + SC7A_A0,0x10,b3dLutDitherMode<<4);
    #if HDR_3DLUT
...
void ConfigB05(BYTE u8WinIdx)
{
    ...
    //imaxc
    msWrite2Byte(u32BaseAddr + SC7A_A0, 0x3FFF);
    //iminc
    msWrite2Byte(u32BaseAddr + SC7A_A2, 0x0000);
    //idistc_invs
    msWrite4Byte(u32BaseAddr + SC7A_A4, 0x00000000);
    //iminy
    msWrite2Byte(u32BaseAddr + SC7A_A8, 0x0000);
    //imaxy
    msWrite2Byte(u32BaseAddr + SC7A_AA, 0x7FFF);
    //idisty_invs
    msWrite4Byte(u32BaseAddr + SC7A_AC, 0x00000000);
    //b05 controls
    msWrite2Byte(u32BaseAddr + SC7A_B0, 0x0024);
    //uv_post_ofs
    msWrite2Byte(u32BaseAddr + SC7A_B2, 0x0800);
	u16iMaxC = 0x3FFF;
	//u16iMinC = 0x0000;
	u16iMinC = 0xC001;
...
void msSetHDRWindowSize(BYTE u8WinIdx, WORD width, WORD height)
{
    ...
    msWrite2Byte(u32BaseAddr + SC79_10, width);
    msWrite2Byte(u32BaseAddr + SC79_12, height);
...
void API_LoadColorMatrix( BYTE u8WinIdx, Matrix3x3 m)
{
    ...
    if (FormatMatrixToS2D10(m, fmtMatrix))
    {
        ...
        for (idx = 0; idx < 9; idx++)
        {
            msWrite2Byte(cmAddr + idx * 2, fmtMatrix[idx]);
            HDR_PrintData("0x%x ", fmtMatrix[idx]);
        }
    }

    //csa2csb round
    msWriteByteMask(u32BaseAddr + SC7B_0E, BIT2, BIT2);
    //csa2csb shift
    msWriteByteMask(u32BaseAddr + SC7B_0F, BIT3, BIT2|BIT3);
    //Write Matrix
    //WriteColorMatrix(u8WinIdx, fmtMatrix);
    //offset
    msWrite2Byte(u32BaseAddr + SC7B_80, 0x0000);
    msWrite2Byte(u32BaseAddr + SC7B_82, 0x0000);
    msWrite2Byte(u32BaseAddr + SC7B_84, 0x0000);

    //clip min
    msWrite2Byte(u32BaseAddr + SC7B_86, 0x0000);
    msWrite2Byte(u32BaseAddr + SC7B_88, 0x0000);
    msWrite2Byte(u32BaseAddr + SC7B_8A, 0x0000);

    //clip max
    //greg ?? why the defalut value of clip max is 0x9C40.
    //chage to 0xFFFF to avoid color abnormal in bypass mode.
    msWrite2Byte(u32BaseAddr + SC7B_8C, 0xFFFF);
    msWrite2Byte(u32BaseAddr + SC7B_8E, 0xFFFF);
    msWrite2Byte(u32BaseAddr + SC7B_90, 0xFFFF);
}
...
void API_OpenHDRMode(BYTE u8WinIdx)
{
    ...
    msWriteByteMask(SC7A_02,BIT7,BIT7); //reg_uvc_byp_en
    //R2Y
...
void API_CloseHDRMode(BYTE u8WinIdx)
{
    ...
    msSetFGammaCMOnOff(u8WinIdx, FALSE);  
    msSetFixGammaOnOff(u8WinIdx, FALSE);
    msSetFGammaOnOff(u8WinIdx, FALSE);
    msSetPostGammaOnOff(u8WinIdx, FALSE);
...
void API_SetHDREOTFGain(BYTE u8WinIdx, float gain)
{
    ...
    msWriteBit(u32BaseAddr + SC7B_0F, FALSE, BIT2);
    msWriteBit(u32BaseAddr + SC7B_0F, FALSE, BIT3);
    for(idx = 0; idx < 9 ; idx++)
    {
        msWrite2Byte(u32BaseAddr + SC7B_40 + idx * 2, gain3x3[idx]);
    }
}
//#endif
...
{
	UNUSED(u8WinIdx);
	msWrite2ByteMask(SC7B_E0, u16YOffset, 0xFFFF);	 
	msWrite2ByteMask(SC7B_E2, u16YGain, 0xFFFF);	 
	msWrite2ByteMask(SC7B_E4, u16COffset, 0xFFFF);	 
	msWrite2ByteMask(SC7B_E6, u16CGain, 0xFFFF);	 
}
...
void msSetHDRY2RBypass(BYTE u8WinIdx, BOOL u8IsBypass)
{
	...
	if(u8IsBypass)
	{
		msWrite2ByteMask(SC7A_02, BIT2, BIT2);	
	}
	else
	{
		msWrite2ByteMask(SC7A_02, 0, BIT2);	
	}
}
...
void msSetHDRCSCBypass(BYTE u8WinIdx, BOOL u8IsBypass)
{
	...
	if(u8IsBypass)
	{
		msWrite2ByteMask(SC7A_02, BIT4, BIT4);	
	}
	else
	{
		msWrite2ByteMask(SC7A_02, 0, BIT4);	
	}
}
...
void msSetHDRR2YBypass(BYTE u8WinIdx, BOOL u8IsBypass)
{
	...
	if(u8IsBypass)
	{
		msWrite2ByteMask(SC7A_02, BIT6, BIT6);	
	}
	else
	{
		msWrite2ByteMask(SC7A_02, 0, BIT6);	
	}
}
...
void msSetHDRUVCBypass(BYTE u8WinIdx, BOOL u8IsBypass)
{
	...
	if(u8IsBypass)
	{
		msWrite2ByteMask(SC7A_02, BIT7, BIT7);	
	}
	else
	{
		msWrite2ByteMask(SC7A_02, 0, BIT7);	
	}
}
...
void msSetHDRTMOBypass(BYTE u8WinIdx, BOOL u8IsBypass)
{
	...
	if(u8IsBypass)
	{
		msWrite2ByteMask(SC7A_04, BIT2, BIT2);	
	}
	else
	{
		msWrite2ByteMask(SC7A_04, 0, BIT2);	
	}
}
...
{
	UNUSED(u8WinIdx);
	msWrite2ByteMask(SC7B_0E, u16Ctrl, u16Mask);
}
...
void msSetHDRY2R_BT2020(BYTE u8WinIdx, BYTE u8IsEn, BYTE u8CHalf, WORD u16MCr, WORD u16MCb, DWORD u32CrOffset, DWORD u32CbOffset)
{
	...
	msWrite2ByteMask(SC7B_D0, u16Ctrl, 0xFFFF);
	msWrite2ByteMask(SC7B_D2, u16MCr, 0xFFFF);
	msWrite2ByteMask(SC7B_D4, u16MCb, 0xFFFF);
	msWrite2ByteMask(SC7B_D6, u32CrOffset & 0xFFFF, 0xFFFF);
	msWrite2ByteMask(SC7B_D8, u32CrOffset >> 16, 0xFFFF);
	msWrite2ByteMask(SC7B_DA, u32CbOffset & 0xFFFF, 0xFFFF);
	msWrite2ByteMask(SC7B_DC, u32CbOffset >> 16, 0xFFFF);
}
...
void msSetHDRCSC(BYTE u8WinIdx, WORD* pMatrix, WORD* pOffset, WORD* pMin, WORD* pMax)
{
	...
	for(u8Idx = 0; u8Idx < 9; u8Idx++)
	{
		msWrite2ByteMask(SC7B_40 + u8Idx * 2, pMatrix[u8Idx], 0xFFFF);	
	}
	
	for(u8Idx = 0; u8Idx < 3; u8Idx++)
	{
		msWrite2ByteMask(SC7B_80 + u8Idx * 2, pOffset[u8Idx], 0xFFFF);	
	}
	
	for(u8Idx = 0; u8Idx < 3; u8Idx++)
	{
		msWrite2ByteMask(SC7B_86 + u8Idx * 2, pMin[u8Idx], 0xFFFF); 
	}
	
	for(u8Idx = 0; u8Idx < 3; u8Idx++)
	{
		msWrite2ByteMask(SC7B_8C + u8Idx * 2, pMax[u8Idx], 0xFFFF); 
	}
}
...
{
	UNUSED(u8WinIdx);
	msWrite2ByteMask(SC7E_8A, u16M0, 0xFFFF);
	msWrite2ByteMask(SC7E_8C, u16M1, 0xFFFF);
	msWrite2ByteMask(SC7E_8E, u16M2, 0xFFFF);
}
...
void msSetHDRR2Y(BYTE u8WinIdx, WORD* pMatrix, WORD* pOffset, WORD* pMin, WORD* pMax)
{
	...
	for(u8Idx = 0; u8Idx < 9; u8Idx++)
	{
		msWrite2ByteMask(SC7B_52 + u8Idx * 2, pMatrix[u8Idx], 0xFFFF);	
	}

	for(u8Idx = 0; u8Idx < 3; u8Idx++)
	{
		msWrite2ByteMask(SC7B_64 + u8Idx * 2, pOffset[u8Idx], 0xFFFF);	
	}

	for(u8Idx = 0; u8Idx < 3; u8Idx++)
	{
		msWrite2ByteMask(SC7B_6A+ u8Idx * 2, pMin[u8Idx], 0xFFFF);	
	}

	for(u8Idx = 0; u8Idx < 3; u8Idx++)
	{
		msWrite2ByteMask(SC7B_70 + u8Idx * 2, pMax[u8Idx], 0xFFFF); 
	}
}
...
{
	UNUSED(u8WinIdx);
	msWrite2ByteMask(SC7B_E8, u16YOffset, 0xFFFF);	
	msWrite2ByteMask(SC7B_EA, u16YGain, 0xFFFF);	
	msWrite2ByteMask(SC7B_EC, u16COffset, 0xFFFF);	
	msWrite2ByteMask(SC7B_EE, u16CGain, 0xFFFF);	
}
...
void msSetHDR422To444To422(BYTE u8WinIdx, BOOL u8IsBypass444To422, WORD u16CtrlMd, BOOL u8IsEn422)
{
	...
	{
    	msWrite2ByteMask(SC79_4A, BIT0, BIT0);	
	}
	else
	{
    	msWrite2ByteMask(SC79_4A, 0, BIT0);	
	}

	msWrite2ByteMask(SC79_4C, u16CtrlMd, 0xFFFF);	
	
    if (u8IsEn422)
	{
    	msWrite2ByteMask(SC79_4E, BIT4, BIT4);	
	}
	else
	{
    	msWrite2ByteMask(SC79_4E, 0, BIT4);	
	}			
}
...
{
	UNUSED(u8WinIdx);
	msWrite2ByteMask(SC79_0C, u16Ctrl0, 0xFFFF);
	msWrite2ByteMask(SC79_0E, u16Ctrl1, 0xFFFF);

	//Set HDR loacation at IP.
	msWrite2ByteMask(SC79_4E, BIT0, BIT0);
	msWrite2ByteMask(SC79_50,    0, BIT15);
}
