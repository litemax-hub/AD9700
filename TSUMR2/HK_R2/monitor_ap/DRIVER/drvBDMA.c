...
#else
#define DRVBDMA_printData(str, value)
#define DRVBDMA_printMsg(str)
#endif

const U32 crc32_lut[256] =
{
...
DWORD BDMA_Operation(BYTE ch, BYTE SourceId, BYTE DestinId, DWORD dwSourceAddr, DWORD dwDestinAddr, DWORD dwByteCount, DWORD dwPattern, DWORD dwExternPattern)
{
    ...
    msWriteByteMask(REG_100900, _BIT4, _BIT4);
	ForceDelay1ms(1);
    msWriteByteMask(REG_100900, 0x00, _BIT4);
    ...
    msWrite4Byte(REG_100914+uctemp, dwPattern);
    msWrite4Byte(REG_100918+uctemp, dwExternPattern);
    ...
    {
    	SetTimOutConter(10);
        while((bTimeOutCounterFlag) && (!(msReadByte(REG_100902+uctemp)&_BIT3)) && (!(msReadByte(REG_100902+uctemp)&_BIT4)));
        ...
        dwDestinAddr = (DWORD)msRead4Byte(REG_100908+uctemp);
    ...
    {
    	SetTimOutConter(10);
        while((bTimeOutCounterFlag) && (!(msReadByte(REG_100902+uctemp)&_BIT3)));
        if(u16TimeOutCounter == 0)
		{
			DRVBDMA_printMsg("TIMEOUT!!");
		}
