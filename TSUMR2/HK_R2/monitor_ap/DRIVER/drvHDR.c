...
void LoadHDRPostGammaTable(BYTE u8WinIdx);
void LoadHDRPanelColorTempRGBGain(void);
#endif
...
PostGammaDesc _PostGamma = {FALSE, {{0}, {0}, {0}}};
...
#if (CHIP_ID == MST9U3)
double SMPTE2084EOTF(double valE)
{
    static double valO = 0;
    valE = pow(valE, 1.0 / _m2);
    valO = pow((MAX(0, valE - _c1) / (_c2 - _c3 * valE)), (1.0 / _m1)) / _factor;
    if (valO > 1)
        valO = 1;
    return valO;
}
...
//return value range is [0-1]
double SMPTE2084OETF(double valO)
{
    static double valE = 0;
    valO = valO / _maxLumOfST2084;
    //valO = pow((valO * _factor), _m1);
    valO = pow(valO, _m1);
    valE = pow((_c1 + _c2 * valO) / (1.0 + _c3 * valO), _m2);
    return valE;
}

void ResetEOTFFactor(double panelMaxLum)
{
    _factor = panelMaxLum / _maxLumOfST2084;
}

void GetEOTFTable(int entries, WORD *pTable, int bitDepth)
{
    int idx = 0;
    DWORD fmt = 0xFFFFFFFF;
    int oftLvl = 0;
    double oft = 0;
    double ratio = 1;
    int panelMaxLum = _panelMaxLum;

    if (bitDepth < 32)
    {
        fmt = ((DWORD)1 << bitDepth) - (1 << (bitDepth - 8));
    }

    if (_IsEnEotfDarkEnhance)
    {
        if (_panelMaxLum < 350)
        {
            oftLvl = 12;
            ResetEOTFFactor(panelMaxLum);
        }
        else if (_panelMaxLum <= 550)  //baseon PC400, 350~500
        {
            oftLvl = 18;
            panelMaxLum = _panelMaxLum + 110;
            ResetEOTFFactor(panelMaxLum);
        }else if (_panelMaxLum <= 800)//baseon PC600 550~800
        {
            oftLvl = 20;
            panelMaxLum = _panelMaxLum + 150;
            ResetEOTFFactor(panelMaxLum);
        }else if (_panelMaxLum > 800) //baseon PC1000 800~1200
        {
            oftLvl = 24;
            panelMaxLum = _panelMaxLum + 200;
            ResetEOTFFactor(panelMaxLum);
        }
        oft = (double)oftLvl / entries;
        ratio = (double)(entries - oftLvl) / entries;
    }
    ...
    for (idx = 0; idx < entries; idx++)
    {
        ...
        {
        if (idx > 0)
            pTable[idx] = (WORD)(SMPTE2084EOTF((double)idx * ratio / (entries - 1) + oft)* fmt);
        else
            pTable[idx] = 0;
    	 }else{
        pTable[idx] = (WORD)(SMPTE2084EOTF((double)idx / (entries - 1)) * fmt);
    	 }
        if (pTable[idx] > fmt)
        {
            pTable[idx] = fmt;
...
void ToneMapping(ToneMappingDesc tmd, BYTE* pTbl, int tblLen, int tblBitDepth)
{
    int idx = 0;
    DWORD fmt = (1 << tblBitDepth) - 1;
    double smax = SMPTE2084OETF(tmd.srcMaxLum);
    double smin = SMPTE2084OETF(tmd.srcMinLum);
    double tmax = SMPTE2084OETF(tmd.trgMaxLum);
    double tmin = SMPTE2084OETF(tmd.trgMinLum);
    double crush = tmin; //or smin or tmin TBD...
    double mid = SMPTE2084OETF(tmd.MaxFALL);
    double clip = SMPTE2084OETF(tmd.MaxCLL);

    double s2tRatio = sqrt((tmax- tmin)/(smax-smin));
    double slope = sqrt(1.0 / s2tRatio);
    double rolloff = (1.0 / 3);
    double slope_rolloff = slope / rolloff;
    double key = (mid - crush) / (clip - crush);
    double shift = mid * (1.0 - s2tRatio) * (2 * key);

    double min = MAX(crush - shift, tmin);
    double max = MIN(clip - shift, tmax);
    //double tmid = Max / 2;// mid - shift;
    double tmid = mid - shift;
    //double tmid = mid - shift;
    //double tmid = 250f / 1023;
    double x1 = pow(crush, slope_rolloff);
    double x2 = pow(mid, slope_rolloff);
    double x3 = pow(clip, slope_rolloff);
    double t1 = pow(min, 1.0 / rolloff);
    double y2 = pow(tmid, 1.0 / rolloff);
    double y3 = pow(max, 1.0 / rolloff);

    double temp = x3 * y3 * (x1 - x2) + x2 * y2 * (x3 - x1) + x1 * t1 * (x2 - x3);
    double c1 = (x2 * x3 * (y2 - y3) * t1 - x1 * x3 * (t1 - y3) * y2 + x1 * x2 * (t1 - y2) * y3) / temp;
    double c2 = (-(x2 * y2 - x3 * y3) * t1 + (x1 * t1 - x3 * y3) * y2 - (x1 * t1 - x2 * y2) * y3) / temp;
    double c3 = ((x3 - x2) * t1 - (x3 - x1) * y2 + (x2 - x1) * y3) / temp;
    ...
    {
        Io = (double)pTbl[idx] / fmt;
        Im = (c1 + c2 * pow(Io, slope_rolloff)) / (1 + c3 * pow(Io, slope_rolloff));
...
#if 0
//------------------------------------------------------------------
void UpdateToneMappingCurve(BYTE u8WinIdx, ToneMappingDesc tmd)
{
#if(CHIP_ID == MST9U4)
    //WORD defCurve[512] = {0};
    xdata DWORD tmoCurve[512] = {0};
    //const int tblLen = 512;
    //const int tblBitDepth = 12;
    //GetGammaCurve(tblLen, tblBitDepth, 1, tmoCurve);
    //memcpy(tmoCurve, defCurve, sizeof(defCurve));
#else
     //default
    //BYTE defCurve[16] = {0x08, 0x18, 0x28, 0x38, 0x48, 0x58, 0x68, 0x78, 0x88, 0x98, 0xA8, 0xB8, 0xC8, 0xD8, 0xE8, 0xF8};
    //strengh dark side
    BYTE tmoCurve[16] = {0x0C, 0x1B, 0x2A, 0x39, 0x48, 0x58, 0x68, 0x78, 0x88, 0x98, 0xA8, 0xB8, 0xC8, 0xD8, 0xE8, 0xF8};

    //streng dark side 2
    //BYTE tmoCurve[16] = {0x12, 0x21, 0x2A, 0x39, 0x48, 0x58, 0x68, 0x78, 0x88, 0x98, 0xA8, 0xB8, 0xC8, 0xD8, 0xE8, 0xF8};
    const int tblLen = 16;
    const int tblBitDepth = 8;
    if (_pref.lcd.IsValid)
    {
        memcpy(tmoCurve, _pref.lcd.lumaCurve, 16);
    }
    //memcpy(tmoCurve, defCurve, sizeof(defCurve));
#endif

    xdata int idx = 0;
    //The tone mapping curve must be monotonically increasement
    xdata bool IsIncrementCurve = true;
    if (tmd.MaxCLL >= tmd.MaxFALL)
    {
        #if(CHIP_ID == MST9U4)
	    if((_pPref != NULL)&&(_pref.tmd12P.IsValid))
	    {
	        //_HDR_ToneMappingGen_12pt_int(_pref.tmd12P.srcMax, _pref.tmd12P.trgMax, _pref.tmd12P.IsAuto, _pref.tmd12P.srcNits, _pref.tmd12P.trgNits, tmoCurve);
	    }
        else
        {
            //_MDrv_HDR_ToneMappingCurveGen(&_hdrTmd, tmoCurve);
	    }
        //MDrv_HDR_ToneMappingCurveGen_Mod(&hdrTmd, tmoCurve);
        if (_pref.lcd.IsValid)
        {
            //Greg : TBD
            //EN_WINDOW_COLOR  eColor = eWIN_COLOR_RGB;
            //msAPIWinGetColorSpaceWindow(u8WinIdx, &eColor);
            //msDrvSetDLCCurve(u8WinIdx, _pref.lcd.lumaCurve);
            //msDrvSetDLCEnable(u8WinIdx, TRUE, eColor);
        }

        #else
        ToneMapping(tmd, tmoCurve, tblLen, tblBitDepth);
        #endif
    }else{
        HDR_PrintMsg("HDR Warning : TMO params is wrong.");
        return;
    }

    #if(CHIP_ID == MST9U3)
    for(idx = 0; idx < (tblLen - 1); idx++)
    {
        if (tmoCurve[idx + 1] >= tmoCurve[idx])
        {
            IsIncrementCurve = true;
        }else{
            IsIncrementCurve = false;
            break;
         }
    }
    #else
	UNUSED(idx);
    #endif
    if (IsIncrementCurve == false)
    {
        HDR_PrintMsg("HDR Warning : TMO curve is wrong.");
        return;
    }


    API_LoadTMOCurve(u8WinIdx, tmoCurve);
}
#endif
...
SMPTE2086Metadata msGetST2086MetadataByPort(BYTE u8WinIdx, BYTE portNum)
{
    ...
    //
    //Greg : Only DP has packet ready status.
    if (INPUT_IS_DISPLAYPORT(SrcInputType) && IsPktReady)
    {
    ...
    else if(INPUT_IS_HDMI(SrcInputType)
    ){
        //HDMI
        metadata.Version = packet[0];
        metadata.Length = packet[1];
...
void msClearHDRUserPref(BYTE u8WinIdx)
{
    ...
    if (_pPref != NULL)
    {
        if (_pref.sharpd.IsValid)
	{
	     #if (CHIP_ID == MST9U4)
            UNUSED(u8WinIdx);
            //Greg : Color API
            //msDrvAdjustPeakingSharpness(u8WinIdx, 0, 0);
            #else
	     //ST_COMBO_COLOR_FORMAT cf = msAPI_combo_IPGetColorFormat((EN_COMBO_IP_SELECT)(msGetHDRPortIPMuxByWin(u8WinIdx)));

	     if (COMBO_COLOR_FORMAT_RGB == _cf.ucColorType)
	     {
	         msDrvAdjustSharpness(u8WinIdx, 0, 0, eWIN_COLOR_RGB);
	     }else{
	         msDrvAdjustSharpness(u8WinIdx, 0, 0, eWIN_COLOR_YUV);
	     }
	     #endif
        }
...
void msExeHDRUserPref(BYTE u8WinIdx)
{
    ...
    {
       //Greg : Color API
       //API_SetUserPrefLumaCurve(_pref.lcd.lumaCurve);
	#if (ENABLE_HDR_CFD)
		msDrvSetDLCCurve(u8WinIdx, _pref.lcd.lumaCurve);
		msDrvSetDLCEnable(u8WinIdx, TRUE, eWIN_COLOR_YUV);
	#endif
    }else{
       //Greg : Color API
       //API_SetUserPrefLumaCurve(_defaultLumaCurve);
    }

    #if 0
    _hdrTmd.slope = 1582;
    _hdrTmd.rolloff = 12288;
    if (_pref.tmd.IsValid)
    {
        _hdrTmd.smin = _pref.tmd.smin;
        _hdrTmd.smed = _pref.tmd.smed;
        _hdrTmd.smax = _pref.tmd.smax;
        _hdrTmd.tmin = _pref.tmd.tmin;
        _hdrTmd.tmed = _pref.tmd.tmed;
        _hdrTmd.tmax = _pref.tmd.tmax;
    }else{
        //default tmo settings
        _hdrTmd.smin = 17;
        _hdrTmd.smed = 330;
        _hdrTmd.smax = 920;
        _hdrTmd.tmin = 17;
    ...
    if(_pref.uvcd.IsValid)
    {
        API_ConfigB04(u8WinIdx, _pref.uvcd.saturation, _pref.uvcd.highTh);
    }else{
        //default UVC settings
        API_ConfigB04(u8WinIdx, 0x1000, 0x20000);
    }

    if (_pref.sharpd.IsValid)
    {
        //Greg : Color API
        //msDrvAdjustPeakingSharpness(u8WinIdx, _pref.sharpd.Level, _pref.sharpd.SharpOrBlur);
    }

    if(_pref.eotfd.IsValid)
    {
        //SetClampEOTF(_pref.eotfd.IsEnClampEOTF, _pref.eotfd.clampNits, _pref.eotfd.clampEOTFType);
    }else{
        //SetClampEOTF(FALSE, 10000, 0);
    }

    //Greg : CFD
    #if 0
    if(_pref.tmd12P.IsValid == TRUE)
    {
        BOOL IsUseCustomTMO = FALSE;
        if (_pref.tmd12P.IsAuto == FALSE)
        {
            IsUseCustomTMO = TRUE;
        }else{
            setPanelMaxLum(_panelMaxLum);
        }
        setHdrCustomTMO(IsUseCustomTMO, _pref.tmd12P.srcNits, _pref.tmd12P.trgNits);
    }else{
        setPanelMaxLum(_panelMaxLum);
        setHdrCustomTMO(FALSE, NULL, NULL);
    }
    #endif
    ...
    if (_pref.sharpd.IsValid)
    {
        ...
        ST_COMBO_COLOR_FORMAT cf = msAPI_combo_IPGetColorFormat((EN_COMBO_IP_SELECT)(msGetHDRPortIPMuxByWin(u8WinIdx)));
        //cf.ucColorType
        if (COMBO_COLOR_FORMAT_RGB == cf.ucColorType)
        {
            msDrvAdjustSharpness(u8WinIdx, _pref.sharpd.Level, _pref.sharpd.SharpOrBlur, eWIN_COLOR_RGB);
        }else{
            msDrvAdjustSharpness(u8WinIdx, _pref.sharpd.Level, _pref.sharpd.SharpOrBlur, eWIN_COLOR_YUV);
        }
        #endif
    }
    #endif
    ...
    #if (CHIP_ID == MST9U4)
    {
        ...
        if (_pref.IHCd.IsValid)
        {
            ...
            for (idx = 1; idx<=6; idx++)
            {
                msDrvAdjustVIPHue(u8WinIdx , _pref.IHCd.IHC[idx], idx);
            }
            msAPI_VIPHueEnable(u8WinIdx, TRUE);
        }
        ...
        if (_pref.IBCd.IsValid)
        {
            ...
            for (idx = 1; idx<=6; idx++)
            {
                //msDrvAdjustCustomBrightness(u8WinIdx , _pref.IBCd.IBC[idx], idx);
                while(u16ScalerMask)
                {
                    if(u16ScalerMask & BIT0)
                    {
                        ...
                        {
                            msWriteByteMask(u32BaseAddr + SC1C_22 + idx, _pref.IBCd.IBC[idx], 0x3F);
                        }
                        else
                        {
                            msWriteByteMask(u32BaseAddr + SC5D_32 + idx, _pref.IBCd.IBC[idx], 0x3F);
            ...
            msDrvCustomBrightnessEnable(u8WinIdx, TRUE);
        }
        ...
        if (_pref.ICCd.IsValid)
        {
            ...
            for (idx = 1; idx<=6; idx++)
            {
                //msDrvAdjustCustomSaturation(u8WinIdx , _pref.ICCd.ICC[idx], idx);
                while(u16ScalerMask)
                {
                    if(u16ScalerMask & BIT0)
                    {
                        ...
                        if( (u8ScalerIdx % 2) == 1)// Main Win
                        {
                            ...
                            if(_pref.ICCd.ICC[idx]&0x80)
                            {
                                msWrite2ByteMask(u32BaseAddr + SC2B_D2,(_BIT0<<idx) ,(_BIT0<<idx));
                            }
                            else
                            {
                                msWrite2ByteMask(u32BaseAddr + SC2B_D2,0 ,(_BIT0<<idx));
                            }

                            msWriteByteMask((u32BaseAddr + SC2B_C2+idx),_pref.ICCd.ICC[idx] ,0x0F);
                        ...
                        else
                        {
                            ...
                            if(_pref.ICCd.ICC[idx]&0x80)
                            {
                                msWrite2ByteMask(u32BaseAddr + SC2B_D4,(_BIT0<<idx) ,(_BIT0<<idx));
                            }
                            else
                            {
                                msWrite2ByteMask(u32BaseAddr + SC2B_D4,0 ,(_BIT0<<idx));
                            }

                            msWriteByteMask(u32BaseAddr + SC2B_C2+idx,(_pref.ICCd.ICC[idx]<<4) ,0xF0);
            ...
            msDrvCustomSaturationEnable(u8WinIdx, TRUE);
        }
        #endif
    }
    #else
    {
        if (_pref.IHCd.IsValid)
        {
            ...
            for (idx = 0; idx<6; idx++)
            {
                msAPI_AdjustCustomHue(u8WinIdx , _pref.IHCd.IHC[idx], idx);
            }
            msAPI_CustomHueEnable(u8WinIdx, TRUE);
        }
        ...
        if (_pref.IBCd.IsValid)
        {
            ...
            for (idx = 0; idx<6; idx++)
            {
                msAPI_AdjustCustomBriRGBMode(u8WinIdx , _pref.IBCd.IBC[idx], idx);
            }
            msAPI_CustomBriEnableRGBMode(u8WinIdx, TRUE);
        }
        ...
        if (_pref.ICCd.IsValid)
        {
            ...
            for (idx = 0; idx<6; idx++)
            {
                msAPI_AdjustCustomSatRGBMode(u8WinIdx , _pref.ICCd.ICC[idx], idx);
            }
            msAPI_CustomSatEnableRGBMode(u8WinIdx, TRUE);
        }
    }
    #endif
}
...
void SetupHDRMode(BYTE u8WinIdx, HDR_Level lvl, SMPTE2086Metadata metadata)
{
    ...
    if (HDR_RESUME == lvl)
    {
        lvl = _lastHDRLvl[u8WinIdx];
        _lastHDRLvl[u8WinIdx] = HDR_OFF;
    }
    ...
    if ((lvl != _lastHDRLvl[u8WinIdx]) || (_IsPanelInfoChanged == TRUE))
    {
        ...
        #if (ENABLE_HDR_CFD)
        if (_pref.tmd12P.IsValid)
        {
            WAIT_V_INPUT_BLANKING_START(u8WinIdx);
            #if !HDR_AUTODOWNLOAD
            LoadHDRPostGammaTbl();
            #endif
            drvCFDHandler(u8WinIdx, 0);
        }else
        #endif
        {
            ...
            //Gamma Calibration
            LoadHDRPostGammaTable(u8WinIdx);

            //Color Gamut Calibration
            if(_pref.hdrColorTemp.IsValid)
            {
                HDR_PrintMsg("HDR Color Temp from User Pref is referenced.");
                LoadHDRPanelColorTempRGBGain();
            }
	    ...
	    LoadXVyccLUT(u8WinIdx);
            ...
            #if 0//!HDR_AUTODOWNLOAD
            LoadHDRPostGammaTbl();
            #endif
            //LoadPanelGammaTable(u8WinIdx);
        }
    }
}
...
{
    //Greg : Combo API
    if (_lastHDRLvl[u8WinIdx] == (int)HDR_DEMO)
    {
...
HDR_Level msHDRHandler(BYTE u8WinIdx, HDR_Level lvl)
{
    ...
    switch((int)lvl)
    {
        ...
        case HDR_DEMO:
        {
            ...
            SetupHDRMode(u8WinIdx, HDR_LOW, GetDemoMetadata());
        }break;
        ...
        case HDR_HIGH:
        {
            ...
            {
                SetupHDRMode(u8WinIdx, lvl, _metadata);
            }
        }break;

        case HDR_UPDATETMO:
               #if (ENABLE_HDR_CFD)
               if(_lastHDRLvl != HDR_OFF)
               {
                    WAIT_V_INPUT_BLANKING_START(u8WinIdx);
                    msExeHDRUserPref(u8WinIdx);
                    drvCFDHandler(u8WinIdx, 1);
                    drvCFDHandler(u8WinIdx, 2);
                }
               #endif
         break;
...
#if ENABLE_ACHDR_FUNCTION
void msSetHDRPostGamma(BYTE *pGammadata)
{
    if (NULL != pGammadata)
    {
...
void LoadHDRPostGammaTable(BYTE u8WinIdx)
{
    if (_PostGamma.IsValid)
    {
        BYTE *StoredPostGamma[][3] =
        {
            {
                _PostGamma.mPostGamma[0],
                _PostGamma.mPostGamma[1],
                _PostGamma.mPostGamma[2],
            },
        };
...
void LoadHDRPanelColorTempRGBGain(void)
{
    if (_HDRColorTemp.IsValid)
    {
        HDR_PrintData("HDR Color Temp R: %x", _HDRColorTemp.RGBGain[0]);
        HDR_PrintData("HDR Color Temp G: %x", _HDRColorTemp.RGBGain[1]);
        HDR_PrintData("HDR Color Temp B: %x", _HDRColorTemp.RGBGain[2]);
        msSetRGBContrast(MAIN_WINDOW, 0x80, _HDRColorTemp.RGBGain[0] << 3,
        _HDRColorTemp.RGBGain[1] << 3, _HDRColorTemp.RGBGain[2] << 3);
        msAdjustPostRGBContrast(MAIN_WINDOW);
    }
}
