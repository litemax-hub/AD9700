...
BOOL mStar_FblOv(void)
{
    if(NULL != g_fpFblOv)
        return g_fpFblOv();
    return FRAME_BFF_SEL; // 0 : FRAME_BUFFER, 1 : FRAME_BFFLESS
}
...
//setup wImgSizeOutH & wImgSizeOutV ---end
    ...
    if((g_SetupPathInfo.ucMemFormat == MEM_FMT_422)||(g_SetupPathInfo.ucMemFormat == MEM_FMT_VC_10to6))
        g_SetupPathInfo.wFieldMode = 0x300;
    else
        g_SetupPathInfo.wFieldMode = 0;

    if(!g_SetupPathInfo.bOverrideSCPathMode)
    {
        if ((( mStar_FblOv() == FRAME_BFFLESS ) && ( g_SetupPathInfo.ucSCFmtIn == SC_FMT_IN_NORMAL ) && IS_INPUT_VFREQ_IN_PANEL_RANGE())
            || InputALLMFlag || IS_HDMI_FREESYNC() || IS_DP_FREESYNC() || (!mStar_FrameBufferSizeCheck()))
...
#ifdef TSUMR2_FPGA
        if(InputALLMFlag || IS_HDMI_FREESYNC() || IS_DP_FREESYNC())
            g_SetupPathInfo.ucSCPathMode = SC_PATH_MODE_0;
        else
            g_SetupPathInfo.ucSCPathMode = SC_PATH_MODE_1;
#endif
...
static void msSetupPathConfig(void)
{
    ...
    if(g_SetupPathInfo.ucSCPathMode == SC_PATH_MODE_0)
    {
        ...
        msWriteBit(SC13_0A, 0, BIT0);  //disable scmi
    }
    else
    {
        msWriteByteMask(SC0D_60, 0, BIT0);//HSP, VSP, MRW
        msWriteBit(SC13_0A, BIT0, BIT0);  //enable scmi
    }
#else
    ...
    switch(g_SetupPathInfo.ucSCPathMode)
    {
        case SC_PATH_MODE_0:
            ...
            msWriteByteMask(SC13_04, IP2SCMI2VSP, 0x33);
            msWriteBit(SC13_0A, 0, BIT0);  //disable scmi
            break;
        case SC_PATH_MODE_1://MRW->VSP->HSP
        ...
        case SC_PATH_MODE_2://HSP->MRW->VSP
            ...
            if(g_SetupPathInfo.bFBMode)
                ...
                msWriteByteMask(SC0D_62, 0x00, BIT0|BIT1|BIT2|BIT4);
            msWriteByteMask(SC13_04, HSP2SCMI2VSP, 0x33);
            msWriteBit(SC13_0A, BIT0, BIT0);  //enable scmi
            break;
        case SC_PATH_MODE_3://VSP->MRW->HSP
            ...
            if(g_SetupPathInfo.bFBMode)
                ...
                msWriteByteMask(SC0D_62, 0x00, BIT0|BIT1|BIT2|BIT4);
            msWriteByteMask(SC13_04, VSP2SCMI2HSP, 0x33);
            msWriteBit(SC13_0A, BIT0, BIT0);  //enable scmi
            break;
        case SC_PATH_MODE_4://HSP->VSP->MRW
            msWrite2Byte(SC0D_60, FIFO_HSP_VSP_MRW_FIFO);
            msWriteByteMask(SC0D_62, 0x00, BIT0|BIT1|BIT2|BIT4);
            msWriteByteMask(SC13_04, VSP2SCMI2OP1, 0x33);
            msWriteBit(SC13_0A, BIT0, BIT0);  //enable scmi
            break;
        case SC_PATH_MODE_5://MRW->HSP->VSP
            ...
            if(g_SetupPathInfo.bFBMode)
                ...
                msWriteByteMask(SC0D_62, 0x00, BIT0|BIT1|BIT2|BIT4);
            msWriteByteMask(SC13_04, IP2SCMI2HSP, 0x33);
            msWriteBit(SC13_0A, BIT0, BIT0);  //enable scmi
            break;
        case SC_PATH_MODE_6://VSP->HSP->MRW
            msWrite2Byte(SC0D_60, FIFO_VSP_HSP_MRW_FIFO);
            msWriteByteMask(SC0D_62, 0x00, BIT0|BIT1|BIT2|BIT4);
            msWriteByteMask(SC13_04, HSP2SCMI2OP1, 0x33);
            msWriteBit(SC13_0A, BIT0, BIT0);  //enable scmi
            break;
        default:
            break;
...
{
    #if 1
    if( g_SetupPathInfo.ucMemFormat == MEM_FMT_VC_10to8 )
    {
        msWriteByteMask(SC06_02, 0, _BIT0);// Enable video compress
        msWriteByteMask(SC06_20, _BIT0, _BIT0);// Enable 8bit video compress
    }
    else if( g_SetupPathInfo.ucMemFormat == MEM_FMT_VC_10to6 )
    {
        msWriteByteMask(SC06_02, 0, _BIT0);// Enable video compress
        msWriteByteMask(SC06_20, 0, _BIT0);// disable 8bit video compress
    }
    else
    {
        msWriteByteMask(SC06_02, _BIT0, _BIT0);// Disable video compress
    }
    #else
...
//VDE
 ...
 //20131017 for bottom side display abnormal while 832x624,800x600 and 1024x768 timing
    ...
    u16TargetBitRate = (WORD)(g_sPnlInfo.u16Width * 5);
    u16MRWLineFetch = ((WORD)g_sPnlInfo.u16Width * 5 ) / 128 + 1;
#else
    msWriteByte(SC01_44, 0xC0);//Compression target bit:Format (xxx.xxxxx)
    u16TargetBitRate = (WORD)(g_sPnlInfo.u16Width * 6);
    u16MRWLineFetch = ((WORD)g_sPnlInfo.u16Width * 6 ) / 128+ 1;
#endif
...
static float msGetMemPixByte(WORD u16FieldMode, BYTE u8MemFormat)
{
    float fPixByte;

    if( u16FieldMode == 0x300 )
        fPixByte = 2;
    else if( (u16FieldMode == 0x311)
        || (u16FieldMode == 0x388)
        || (u16FieldMode == 0x399)
        || (u16FieldMode == 0x3AA)
        || (u16FieldMode == 0x3BB) )
        fPixByte = 3;
    else if( (u16FieldMode == 0x322)
        || (u16FieldMode == 0x333) )
        fPixByte = 4;
    else if( (u16FieldMode == 0x344)
        || (u16FieldMode == 0x355)
        || (u16FieldMode == 0x366) )
        fPixByte = 2.5;
    else if( u16FieldMode == 0x3CC )
        fPixByte = 1;
    else if( u16FieldMode == 0x3DD )
        fPixByte = 1.5;
    else if( u16FieldMode == 0x3FF )
        fPixByte = 4.5;
    else if( u8MemFormat == MEM_FMT_422 )
        fPixByte = 2;
    else if( u8MemFormat == MEM_FMT_444_8
        || (u8MemFormat == MEM_FMT_VC_10to8) )
        fPixByte = 3;
    else if( u8MemFormat == MEM_FMT_444_10 )
        fPixByte = 4;
    else if( u8MemFormat == MEM_FMT_VC_10to6 )
        fPixByte = 2;
    else
        fPixByte = 3;
...
static void msSetMemFormat(void)
{
    if(!g_SetupPathInfo.bFBMode) //FBL case
    {
        msWriteBit(SC12_0E, BIT0, BIT0);  //IPM write request disable
        msWriteBit(SC12_09, BIT3, BIT3);  //OPM read request disable
        msWriteBit(SC13_40, BIT0, BIT0);  //disable IPM align
    }
    else
    {
        msWriteBit(SC12_0E, BIT0, BIT0);  //IPM write request disable
        msWriteBit(SC12_0E, 0, BIT0);
        msWriteBit(SC12_09, 0, BIT3);
        msWriteBit(SC13_40, 0, BIT0);
    }

    if( g_SetupPathInfo.ucMemFormat == MEM_FMT_NO_USE )
    {
        msWriteByteMask(SC12_02, 0, _BIT5|_BIT4); // default setting
        msWrite2Byte(SC12_04, 0x0000);

        msWriteByteMask(SC13_20, _BIT0, _BIT0); // Enable  444 -> 422 filter
        msWriteByteMask(SC13_20, 0, _BIT7|_BIT6);     // 422 -> 444 OFF
    }
    else if( ( g_SetupPathInfo.wFieldMode == 0x300 )
        || ( g_SetupPathInfo.wFieldMode == 0x344 )
        || ( g_SetupPathInfo.wFieldMode == 0x355 )
        || ( g_SetupPathInfo.wFieldMode == 0x366 )
        || ( g_SetupPathInfo.wFieldMode == 0x388 )
        || ( g_SetupPathInfo.wFieldMode == 0x399 )
        || ( g_SetupPathInfo.wFieldMode == 0x3AA )
        || ( g_SetupPathInfo.wFieldMode == 0x3BB )
        || ( g_SetupPathInfo.wFieldMode == 0x3CC )
        || ( g_SetupPathInfo.wFieldMode == 0x3DD )
        || ( g_SetupPathInfo.ucMemFormat == MEM_FMT_VC_10to6 ) )
    {
        msWriteByteMask(SC12_02, 0, _BIT5|_BIT4); // 422 format
        msWrite2Byte(SC12_04, g_SetupPathInfo.wFieldMode);

        msWriteByteMask(SC13_20, 0, _BIT0); // Enable 444 -> 422 filter
        msWriteByteMask(SC13_20, _BIT6, _BIT7|_BIT6); // 422 -> 444 OFF
    }
    else if( g_SetupPathInfo.wFieldMode == 0x311 )
    {
        msWriteByteMask(SC12_02, 0, _BIT5|_BIT4);
        msWrite2Byte(SC12_04, g_SetupPathInfo.wFieldMode);

        msWriteByteMask(SC13_20, _BIT0, _BIT0); // Disable 444 -> 422 filter
        msWriteByteMask(SC13_20, 0, _BIT7|_BIT6); // 422 -> 444 OFF
    }
    else if( ( g_SetupPathInfo.wFieldMode == 0x322 )
        || ( g_SetupPathInfo.wFieldMode == 0x333 ) )
    {
        msWriteByteMask(SC12_02, 0, _BIT5|_BIT4);
        msWrite2Byte(SC12_04, g_SetupPathInfo.wFieldMode);
    ...
    else if( g_SetupPathInfo.ucMemFormat == MEM_FMT_422 )
    {
        msWriteByteMask(SC12_02, 0, _BIT5|_BIT4); // 422 format
        msWrite2Byte(SC12_04, 0x0000);
    ...
    else if( g_SetupPathInfo.ucMemFormat == MEM_FMT_444_8 )
    {
        msWriteByteMask(SC12_02, _BIT4, _BIT5|_BIT4); // 444 8 bit format
        msWrite2Byte(SC12_04, 0x0000);

        msWriteByteMask(SC13_20, _BIT0, _BIT0); // disable  444 -> 422 filter
        msWriteByteMask(SC13_20, 0, _BIT7|_BIT6);     // 422 -> 444 OFF
    }
    else if( g_SetupPathInfo.ucMemFormat == MEM_FMT_444_10 )
    {
        msWriteByteMask(SC12_02, _BIT5, _BIT5|_BIT4); // 444 10 bit format
        msWrite2Byte(SC12_04, 0x0000);

        msWriteByteMask(SC13_20, _BIT0, _BIT0); // Enable  444 -> 422 filter
        msWriteByteMask(SC13_20, 0, _BIT7|_BIT6);     // 422 -> 444 OFF
    }
    else if( g_SetupPathInfo.ucMemFormat == MEM_FMT_VC_10to8 )
    {
        msWriteByteMask(SC12_02, _BIT4, _BIT5|_BIT4); // 444 8 bit format
        msWrite2Byte(SC12_04, 0x0000);
...
static void msSetMemFetch(void)
{
    DB_W2B(SC12_1E, ALIGN_2(g_SetupPathInfo.wMRWSizeInH));          // input fetch number
    DB_W2B(SC12_2E, ALIGN_2(g_SetupPathInfo.wMRWSizeOutH));         // output fetch number
    DB_W2B(SC12_30, g_SetupPathInfo.wMRWSizeInV|BIT15);             // input vertical limitation
    DB_W2B(SC13_02, g_SetupPathInfo.wMRWSizeOutV);                  // output vertical limitation
}

static void msMemMRQ(BOOL bForceBlock)
{
    DWORD u32FBAddrStart = (DWORD)MIU_FB_ADDR_START>>5;
    DWORD u32FBAddrEnd = (DWORD)MIU_FB_ADDR_END>>5;
    ...
    DB_W4B(SC13_80, u32FBAddrStart);
    DB_W4B(SC13_84, u32FBAddrEnd);
}

static void msSetMemConfig(void)
{
    DWORD u32IpmMemBase, u32OffsetBase, u32MemBase = (DWORD)MIU_FB_ADDR_START;
    WORD u16IpmOffset, ucDelay = 3 + 3 + PANEL_SHIFT_DE_FB_V;// DMA 12us + ip 3 lines + pixel shift
    BYTE u8FieldNum = g_SetupPathInfo.ucFieldNum;
    BYTE u8RWBankMode = 0;
    WORD u16SC12_0607 = msRead2Byte(SC12_06);
    WORD u16SC12_0A0B = msRead2Byte(SC12_0A);
    WORD u16SC13_2021 = msRead2Byte(SC13_20);
    WORD u16SC13_4041 = msRead2Byte(SC13_40);
    float fPixByte = msGetMemPixByte(g_SetupPathInfo.wFieldMode, g_SetupPathInfo.ucMemFormat);

    if( (g_SetupPathInfo.bReadCurFrame) || (!g_SetupPathInfo.bFBMode) )
        u8RWBankMode = 0;
    else
        u8RWBankMode = 0x20;

    // IPM Write Off
    if( (g_SetupPathInfo.wMRWSizeInH == 0)||(!g_SetupPathInfo.bFBMode) )
        msMemMRQ(TRUE);
    else
        msMemMRQ(FALSE);

    // IPM Offset Align
    u16IpmOffset = ALIGN_64(g_SetupPathInfo.wMRWSizeInH);

    // V MIRROR
    if( g_SetupPathInfo.bVFlipMode )
    {
        // ( H * (V-1) )
        u32OffsetBase = ((DWORD)u16IpmOffset * (g_SetupPathInfo.wMRWSizeInV - 1) );
        u32OffsetBase = ((float)u32OffsetBase * fPixByte * u8FieldNum);
    ...
    if( g_SetupPathInfo.bHMirrorMode )
    {
        u16SC12_0607 |= (BIT12);
        u16SC13_4041 |= BIT1;
        if( ( g_SetupPathInfo.ucMemFormat == MEM_FMT_422 )
            && ( g_SetupPathInfo.wFieldMode != 0x311 )
            && ( g_SetupPathInfo.wFieldMode != 0x322 )
            && ( g_SetupPathInfo.wFieldMode != 0x333 )
            )
    ...
    if( CURRENT_SOURCE_IS_INTERLACE_MODE() && (g_SetupPathInfo.bHMirrorMode || g_SetupPathInfo.bVFlipMode) )
        u16SC12_0A0B |= _BIT10;
    else
        u16SC12_0A0B &= ~(_BIT10);

    // set trig
    msWrite2Byte(SC00_AE, g_sPnlInfo.sPnlTiming.u16Vtt-ucDelay);
    // IPM, OPM Base Address / Offset
    DB_W4B(SC12_10, u32IpmMemBase>>5);                   // Main window Input Video Base address 0
    DB_W2B(SC12_1C, u16IpmOffset);                       // input offset
    DB_W4B(SC12_20, u32MemBase>>5);                      // Main window Output Base address 0
    DB_W2B(SC12_2C, u16IpmOffset);                       // output offset
    // FMCLK
    msWriteByteMask(REG_120F66, 0x04, 0x07);

    DB_W2B(SC13_20,u16SC13_2021);
    DB_W2B(SC13_40,u16SC13_4041);
    DB_W2B(SC12_06,u16SC12_0607);
    DB_W2B(SC12_0A,u16SC12_0A0B|_BIT7);
    DB_W2B(SC12_0A,u16SC12_0A0B);
    DB_WBMask(SC12_32, u8FieldNum, 0x1F);
    DB_WBMask(SC12_0F, u8RWBankMode, 0xE0);
}

static void msSetupPathMemory(void)
{
    msSetMemFormat();
    msSetMemFetch();
    msSetMemConfig();
...
static void msSetMemFreeze(BOOL bEnable)
{
    if(bEnable) // Freeze
    {
        msWriteBit(SC12_03, TRUE, _BIT3);
    }
    else // Release
    {
        msWriteBit(SC12_03, FALSE, _BIT3);
        msWriteBit(SC12_0F, FALSE, _BIT3);
    }
}
...
void mStar_SetupPathInit(void)
{
    ...
    g_SetupPathInfo.wImgSizeOutH = g_sPnlInfo.sPnlTiming.u16Width;
    g_SetupPathInfo.wImgSizeOutV = g_sPnlInfo.sPnlTiming.u16Height;
    g_SetupPathInfo.ucMemFormat = MEM_FMT_VC_10to8;
    g_SetupPathInfo.ucFieldNum = FIELD_NUM;
    g_SetupPathInfo.wFieldMode = 0x0;
    g_SetupPathInfo.bReadCurFrame = FALSE;
}

void mStar_SetMemFreeze(BOOL bEnable)
{
    if( g_SetupPathInfo.bFBMode )
        msSetMemFreeze(bEnable);
}

BOOL mStar_FrameBufferDump(DWORD u32DestAddress, size_t u32DestSize, BOOL bIsPixelOrder, BYTE u8ColorFormat)
{
    DWORD u32IpmMemBase = msRead4Byte(SC12_10)<<5, u32MemBaseG, u32MemBaseB, u32MemBaseR, u32MemBaseLSB;
    WORD i, k, u16G, u16B, u16R;
    WORD u16IpmFetch = msRead2Byte(SC12_1E);
    WORD u16IpmOffset = msRead2Byte(SC12_1C);
    WORD u16IpmHeight = msRead2Byte(SC12_30) & 0x7FF;
    BYTE u8FieldNum = msReadByte(SC12_32) & 0x1F, u8FameIndex = 0, u8LSB, j;
    EN_COLOR_FORMAT_TYPE eColorFormat = (EN_COLOR_FORMAT_TYPE)u8ColorFormat;
    BOOL bIsRGB = (eColorFormat == COMBO_COLOR_FORMAT_RGB);
    BOOL bIsYUV422 = (eColorFormat == COMBO_COLOR_FORMAT_YUV_422);
    BOOL bIsYUV420 = (eColorFormat == COMBO_COLOR_FORMAT_YUV_420);

    if( !g_SetupPathInfo.bFBMode )
    {
        MSSCALER_printMsg("Only provide data dump for frame buffer mode!!");
        return FALSE;
    }
    if( MEM_FMT_444_10 != g_SetupPathInfo.ucMemFormat )
    {
        MSSCALER_printMsg("Only provide data dump for MEM_FMT_444_10!!");
        return FALSE;
    }
    if( bIsYUV420 && bIsPixelOrder)
    {
        MSSCALER_printMsg("420 do not support Pixel Order Format!!");
        return FALSE;
    }
    if((3UL * u16IpmFetch * u16IpmHeight >= u32DestSize) && bIsYUV420)
    {
        MSSCALER_printMsg("420 memory size not enough for dump location!!");
        return FALSE;
    }
    else if((4UL * u16IpmFetch * u16IpmHeight >= u32DestSize) && (bIsYUV422))
    {
        MSSCALER_printMsg("422 memory size not enough for dump location!!");
        return FALSE;
    }
    else if(6UL * u16IpmFetch * u16IpmHeight >= u32DestSize)
    {
        MSSCALER_printMsg("444 memory size not enough for dump location!!");
        return FALSE;
    }
    else if( (DRAM_SIZE < (u32DestAddress + u32DestSize)) || (u32DestAddress >= (u32DestAddress + u32DestSize)))
    {
        MSSCALER_printMsg("dump size invalid!!");
        return FALSE;
    }
    mStar_SetMemFreeze(TRUE);
    for(i = 0; i < u16IpmOffset / 64UL * u16IpmHeight ; i++)
    {
        for(k=2UL*i; k<2UL*i+2; k++)
        {
            u32MemBaseG = u32IpmMemBase + 32UL * 4UL * u8FieldNum * k + 32UL * 0UL * u8FieldNum + 32UL * u8FameIndex;
            u32MemBaseB = u32IpmMemBase + 32UL * 4UL * u8FieldNum * k +  32UL * 1UL * u8FieldNum + 32UL * u8FameIndex;
            u32MemBaseR = u32IpmMemBase + 32UL * 4UL * u8FieldNum * k + 32UL * 2UL * u8FieldNum + 32UL * u8FameIndex;
            u32MemBaseLSB = u32IpmMemBase + 32UL * 4UL * u8FieldNum * k + 32UL * 3UL * u8FieldNum + 32UL * u8FameIndex;

            for(j=0;j<32;j++)
            {
                if( (((32UL * k) % u16IpmHeight) + j) > u16IpmFetch )
                    break;
                u8LSB = msMemReadByte(u32MemBaseLSB++);
                u16G = (((WORD)msMemReadByte(u32MemBaseG++))<< 4) | ((u8LSB & 0x03)<<2);//Y
                u16B = (((WORD)msMemReadByte(u32MemBaseB++))<< 4) | (u8LSB & 0x0C);//Cb
                u16R = (((WORD)msMemReadByte(u32MemBaseR++))<< 4) | ((u8LSB & 0x30)>>2);//Cr
                if(bIsPixelOrder)
                {
                    if(bIsYUV422)
                    {
                        //pixel order 422 CbYCrY
                        msMemWrite2Byte(u32DestAddress + 4UL * 32UL * k + 4UL * j + 0UL, (j&1)?u16R:u16B);
                        msMemWrite2Byte(u32DestAddress + 4UL * 32UL * k + 4UL * j + 2UL, u16G);
                    }
                    else if(bIsRGB)
                    {
                        //pixel order format RGB
                        msMemWrite2Byte(u32DestAddress + 6UL * 32UL * k + 6UL * j + 0UL, u16R);
                        msMemWrite2Byte(u32DestAddress + 6UL * 32UL * k + 6UL * j + 2UL, u16G);
                        msMemWrite2Byte(u32DestAddress + 6UL * 32UL * k + 6UL * j + 4UL, u16B);
                    }
                    else
                    {
                        //pixel order format YCbCr
                        msMemWrite2Byte(u32DestAddress + 6UL * 32UL * k + 6UL * j + 0UL, u16G);
                        msMemWrite2Byte(u32DestAddress + 6UL * 32UL * k + 6UL * j + 2UL, u16B);
                        msMemWrite2Byte(u32DestAddress + 6UL * 32UL * k + 6UL * j + 4UL, u16R);
                    }
                }
                else
                {
                    if(bIsYUV420)
                    {
                        //planar format 420 YCbCr
                        msMemWrite2Byte(u32DestAddress + 0UL * u16IpmFetch * u16IpmHeight + 2UL * 32UL * k + 2UL * j, u16G);
                        if(!((k /(u16IpmOffset / 32UL)) & 1))
                        {
                            if(j&1)
                                msMemWrite2Byte(u32DestAddress + 5UL * u16IpmFetch * u16IpmHeight / 2UL + 32UL * k /2UL + (j&0xFE), u16R);
                            else
                                msMemWrite2Byte(u32DestAddress + 4UL * u16IpmFetch * u16IpmHeight / 2UL + 32UL * k /2UL + (j&0xFE), u16B);
                        }
                    }
                    else if(bIsYUV422)
                    {
                        //planar format 422 YCbCr
                        msMemWrite2Byte(u32DestAddress + 0UL * u16IpmFetch * u16IpmHeight + 2UL * 32UL * k + 2UL * j, u16G);
                        if(j&1)
                            msMemWrite2Byte(u32DestAddress + 3UL * u16IpmFetch * u16IpmHeight + 32UL * k + (j&0xFE), u16R);
                        else
                            msMemWrite2Byte(u32DestAddress + 2UL * u16IpmFetch * u16IpmHeight + 32UL * k + (j&0xFE), u16B);
                    }
                    else if(bIsRGB)
                    {
                        //planar format RGB
                        msMemWrite2Byte(u32DestAddress + 0UL * u16IpmFetch * u16IpmHeight + 2UL * 32UL * k + 2UL * j, u16R);
                        msMemWrite2Byte(u32DestAddress + 2UL * u16IpmFetch * u16IpmHeight + 2UL * 32UL * k + 2UL * j, u16G);
                        msMemWrite2Byte(u32DestAddress + 4UL * u16IpmFetch * u16IpmHeight + 2UL * 32UL * k + 2UL * j, u16B);
                    }
                    else
                    {
                        //planar format YCbCr
                        msMemWrite2Byte(u32DestAddress + 0UL * u16IpmFetch * u16IpmHeight + 2UL * 32UL * k + 2UL * j, u16G);
                        msMemWrite2Byte(u32DestAddress + 2UL * u16IpmFetch * u16IpmHeight + 2UL * 32UL * k + 2UL * j, u16B);
                        msMemWrite2Byte(u32DestAddress + 4UL * u16IpmFetch * u16IpmHeight + 2UL * 32UL * k + 2UL * j, u16R);
                    }
                }
            }
        }
    }
    mStar_SetMemFreeze(FALSE);
    return TRUE;
}

void mStar_FblCBAttach(BOOL (*fpFblOv)(void))
{
    g_fpFblOv = fpFblOv;
}

BOOL mStar_FrameBufferSizeCheck(void)
{
    DWORD u32FBAddrStart = (DWORD)MIU_FB_ADDR_START;
    DWORD u32FBAddrEnd = (DWORD)MIU_FB_ADDR_END;
    DWORD u32FBSize;
    float fPixByte = msGetMemPixByte(g_SetupPathInfo.wFieldMode, g_SetupPathInfo.ucMemFormat);
    BOOL bRet = TRUE;
    WORD u16SCMISizeH;
    ...
    if( g_SetupPathInfo.wImgSizeInV < g_SetupPathInfo.wImgSizeOutV )
        u16SCMISizeV = g_SetupPathInfo.wImgSizeInV;
    else
        u16SCMISizeV = g_SetupPathInfo.wImgSizeOutV;

    if( g_SetupPathInfo.wImgSizeInH < g_SetupPathInfo.wImgSizeOutH )
        u16SCMISizeH = g_SetupPathInfo.wImgSizeInH;
    else
        u16SCMISizeH = g_SetupPathInfo.wImgSizeOutH;

    u32FBSize = (DWORD)((float)(ALIGN_64(u16SCMISizeH)) * u16SCMISizeV * fPixByte * g_SetupPathInfo.ucFieldNum);

    if((u32FBAddrStart >= u32FBAddrEnd) || (u32FBSize > (u32FBAddrEnd-u32FBAddrStart)))
    {
        MSSCALER_printMsg("--------------------NO frame buffer size!!!---------------------------");
        MSSCALER_PRINT("u32FBAddrStart : %lx\r\n", u32FBAddrStart);
        MSSCALER_PRINT("u32FBAddrEnd   : %lx\r\n", u32FBAddrEnd);
        MSSCALER_PRINT("u32FBSize      : %lx\r\n", u32FBSize);
        bRet = FALSE;
    }
