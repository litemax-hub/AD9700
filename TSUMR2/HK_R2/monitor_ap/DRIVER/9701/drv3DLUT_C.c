...
BYTE code t3DSRAMMTbl_EXT[4][729] =
{
...
void msDrv3DLut_WaitADLFinished(BYTE u8WinIdx);
void WriteTriggerADL3DLUT(void);
void msDrvClearDMA3DLUTDataBlock(void);
#endif
...
BYTE code t3DLutTbl_S7ColorFmt[3][729] = 
{
    {
...
void msDrvClearDMA3DLUTDataBlock(void)
{
    DWORD u32Gamma_write_ptr = RGB3DLUT_ADL_ADDR;
    DWORD idx;
    for( idx=0; idx<RGB3DLUT_ADL_SIZE; idx++)
    {
...
void WriteTriggerADL3DLUT(void)
{
    msWrite4Byte(SC67_46, (RGB3DLUT_ADL_ADDR>>5)); //base address for client7
    ...
    msDrv3DLut_WaitADLFinished(0);
}

void msDrv3DLut_WaitADLFinished(BYTE u8WinIdx)
{
    UNUSED(u8WinIdx);
    DWORD addrEn = SC67_44;

    TimeOutCounter =50;
    while((!(msReadByte(addrEn) &_BIT0)) && TimeOutCounter){}
...
void ms3DLutLoadTbl(BYTE ColorMode)
{
    ...
    DB_PRINT("RGB3DLUT_ADL_ADDR=%X\n",RGB3DLUT_ADL_ADDR);
    DB_PRINT("RGB3DLUT_ADL_ADDR_END=%X\n",RGB3DLUT_ADL_ADDR+RGB3DLUT_ADL_SIZE);
    BYTE sramSel;
    WORD sramAddr;
    WORD TblBaseAddr;
    WORD value;
    WORD u16Idx = 0;
    BYTE u8SwRatio = 4;
    DWORD u32Lut_write_ptr = RGB3DLUT_ADL_ADDR;
    DWORD u32Addr = 0;
    WORD u16Data = 0;
    BYTE u8Color;
    ...
    msWriteByte(SC14_D0, 0x40); //table gain after SRAM format
    msDrvClearDMA3DLUTDataBlock();
    ...
    for(sramSel=0; sramSel<8; sramSel++)
    {
        for(sramAddr=0; sramAddr<t3DSRAMTblSize[sramSel]; sramAddr++)
        {
            ...
            for(u8Color=0; u8Color<3; u8Color++)
            {
                ...
                u16Data = (WORD)(value*(WORD)u8SwRatio);
                ...
                msMemWrite2ByteMask(u32Addr + u16AllocateBit/8, u16Data<<(u16AllocateBit%8), 0x3FF<<(u16AllocateBit%8)); //channel data
            }
            msMemWriteByteMask(u32Addr + (u8WriteEnableBit/8), 1<<(u8WriteEnableBit%8), 1<<(u8WriteEnableBit%8));
...
void ms3DLutLoadTbl(BYTE ColorMode)
{
    ...
    for(sramSel=0; sramSel<8; sramSel++)
    {
        ...
        for(sramAddr=0; sramAddr<t3DSRAMTblSize[sramSel]; sramAddr++)
        {
            ...
            if (t3DHWGain[ColorMode] == e3dLutDelta8X && (t3DSRAMMTbl_EXT[0][u16Idx/8] & (1 <<(u16Idx % 8))))
            ...
            if (t3DHWGain[ColorMode] == e3dLutDelta8X && (t3DSRAMMTbl_EXT[1][(u16Idx+729)/8] & (1 <<((u16Idx+729) % 8))))
            ...
            if (t3DHWGain[ColorMode] == e3dLutDelta8X && (t3DSRAMMTbl_EXT[2][(u16Idx+729*2)/8] & (1 <<((u16Idx+729*2) % 8))))
...
{
    //BYTE xdata RegValue;
    msWriteByteMask(SC14_C8,(t3DHWGain[ColorMode]*2)<<2,BIT2|BIT3|BIT4);
