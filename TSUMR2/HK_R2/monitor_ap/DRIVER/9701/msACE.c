...
static BOOL _u8IsEnabledMWE = FALSE;
...
/*
     ...
     if(bScalerWin == MAIN_WINDOW)
    {
        ...
        _scWriteByteMask(SC25_03, _BIT3 ,_BIT3);//Fix gamma dither
    }
     else
    {
        //_scWriteByteMask(SC0E_22, _BIT2 ,_BIT2);
        _scWriteByteMask(SC25_23, _BIT3 ,_BIT3);//Fix gamma dither
    }
...
void InitACEVar(void)
{
    ...
    g_sPCRGBMatrix_Range[MAIN_WINDOW][0][0] = g_sPCRGBMatrix_Range[MAIN_WINDOW][1][1] = g_sPCRGBMatrix_Range[MAIN_WINDOW][2][2] = 1024;
    g_sPCRGBMatrix_Range[MAIN_WINDOW][0][1] = g_sPCRGBMatrix_Range[MAIN_WINDOW][1][0] = g_sPCRGBMatrix_Range[MAIN_WINDOW][2][0] =
    g_sPCRGBMatrix_Range[MAIN_WINDOW][0][2] = g_sPCRGBMatrix_Range[MAIN_WINDOW][1][2] = g_sPCRGBMatrix_Range[MAIN_WINDOW][2][1] = 0;

    g_sPCRGBMatrix_Range[SUB_WINDOW][0][0] = g_sPCRGBMatrix_Range[SUB_WINDOW][1][1] = g_sPCRGBMatrix_Range[SUB_WINDOW][2][2] = 1024;
    g_sPCRGBMatrix_Range[SUB_WINDOW][0][1] = g_sPCRGBMatrix_Range[SUB_WINDOW][1][0] = g_sPCRGBMatrix_Range[SUB_WINDOW][2][0] =
    g_sPCRGBMatrix_Range[SUB_WINDOW][0][2] = g_sPCRGBMatrix_Range[SUB_WINDOW][1][2] = g_sPCRGBMatrix_Range[SUB_WINDOW][2][1] = 0;        
...
void SetVideoContrastMatrix(BYTE bScalerWin)//static void SetVideoContrastMatrix()
{
	...
	g_sVideoContrastMatrix[bScalerWin][0][0] = ( (WORD)g_ucVideoRCon * (VideoContrast)) >> 4;
    g_sVideoContrastMatrix[bScalerWin][1][1] = ( (WORD)g_ucVideoGCon * (VideoContrast)) >> 4;
    g_sVideoContrastMatrix[bScalerWin][2][2] = ( (WORD)g_ucVideoBCon * (VideoContrast)) >> 4;
}
    ...
    for(i=0; i!=3; i++)
    {
        for(j=0; j!=3; j++)
        {
            ...
            if( sTmp >= 0 )
            {
                ...
                msWrite2ByteMask( ucAddr, sTmp<<3, 0xFFFF );//from 2.10 -> 2.13
            }
            else
            {
                 if (sTmp < (-4096))
...
void msSetVideoColorMatrix_Contrast(BYTE bScalerWin, BYTE ucContrast)
{
    ...
    if(g_WinColor[bScalerWin] == WIN_COLOR_RGB) return;
...
#define ACC_H_POS       (PANEL_H_START+g_sPnlInfo.u16Width/4)
#define ACC_V_POS       (g_sPnlInfo.u16Height/4)
#define ACC_H_SIZE      g_sPnlInfo.u16Width/2
#define ACC_V_SIZE      g_sPnlInfo.u16Height/2

#define ACC_H_POS_SPLIT       (PANEL_H_START+g_sPnlInfo.u16Width/2)
#define ACC_V_POS_SPLIT       (0)
#define ACC_H_SIZE_SPLIT      g_sPnlInfo.u16Width/2
#define ACC_V_SIZE_SPLIT      g_sPnlInfo.u16Height

#define ACC_H_POS_FULL       (PANEL_H_START)
#define ACC_V_POS_FULL       (0)
#define ACC_H_SIZE_FULL      g_sPnlInfo.u16Width
#define ACC_V_SIZE_FULL      g_sPnlInfo.u16Height
...
void msAdjustSharpness(BYTE bScalerWin ,BYTE ucSharpness, BYTE ucStep)
{
    ...
    UNUSED(ucStep);
    BYTE isBlur = 0;
    BYTE midtone = (MinSharpness+MaxSharpness)/2;
    if(ucSharpness == midtone)
    {
        ucSharpness = 0;
    }
    else if(ucSharpness < midtone)
    {
        isBlur = 1;
        ucSharpness = (midtone - ucSharpness)*MaxSharpness/midtone;
    }
    else
    {
        isBlur = 0;
        ucSharpness = (ucSharpness - midtone)*MaxSharpness/(MaxSharpness-midtone);
    }
 
    if(bScalerWin == MAIN_WINDOW)
    {
        _scWriteByte(SC0B_27, ucSharpness);
        if(isBlur)
           _scWriteByteMask(SC0B_25, BIT3, BIT3);//Blurriest
        else
            _scWriteByteMask(SC0B_25, 0, BIT3);//Sharpest

        _scWriteByteMask( SC0B_20, BIT0	, BIT0 );	//main post peaking enable
    }
    else
    {
        _scWriteByte(SC0B_2F, ucSharpness);
        if(isBlur)
            _scWriteByteMask(SC0B_2D, 0, BIT3);//Blurriest
        else
            _scWriteByteMask(SC0B_2D, BIT3, BIT3);//Sharpest
...
{
	//printf("[msSetRGBContrast]ucContrast:%x\n", ucContrast);
    s_AceInfo[ucWinIndex].sRGBContrast[0] =  ( (WORD)ucRCon * (ucContrast)) >> 4;
    s_AceInfo[ucWinIndex].sRGBContrast[1] =  ( (WORD)ucGCon * (ucContrast)) >> 4;
    s_AceInfo[ucWinIndex].sRGBContrast[2] =  ( (WORD)ucBCon * (ucContrast)) >> 4;
	//printData("[msSetRGBContrast]R 1:%x\n", s_AceInfo[ucWinIndex].sRGBContrast[0]);
	//printData("[msSetRGBContrast]G 1:%x\n", s_AceInfo[ucWinIndex].sRGBContrast[1]);
    //printData("[msSetRGBContrast]B 1:%x\n", s_AceInfo[ucWinIndex].sRGBContrast[2]);
    s_AceInfo[ucWinIndex].sRGBContrast[0] =  ( (WORD)s_AceInfo[ucWinIndex].sRGBContrast[0] *g_ucSubContrast) >> 10;
    s_AceInfo[ucWinIndex].sRGBContrast[1] =  ( (WORD)s_AceInfo[ucWinIndex].sRGBContrast[1] * g_ucSubContrast) >> 10;
    s_AceInfo[ucWinIndex].sRGBContrast[2] =  ( (WORD)s_AceInfo[ucWinIndex].sRGBContrast[2] *g_ucSubContrast) >> 10;


	//printData("[msSetRGBContrast]R:%x\n", s_AceInfo[ucWinIndex].sRGBContrast[0]);
...
void msACECSCControl(void)
{
    ...
    if(cf.ucColorType == COMBO_COLOR_FORMAT_RGB){
        msACESetRGBMode(ENABLE);
    }
    else{
        msACESetRGBMode(DISABLE);
    }
...
void msACEEnableDering(BYTE bScalerWin, BOOL bEnable)
{
    // Main & Sub
    if(MAIN_WINDOW == bScalerWin)
        msWriteByteMask(SC0B_27, bEnable?(BIT6|BIT7):0, (BIT6|BIT7));
    else
        msWriteByteMask(SC0B_29, bEnable?(BIT6|BIT7):0, (BIT6|BIT7));
...
void msACESetVIPAllColor(void)
{
	...
	for (bScalerWin = 0; bScalerWin < 2; bScalerWin++)
	{
		//2d Peaking
		msAdjustSharpness(bScalerWin , 0x3F, 1);
		//H/Vdering
		msACEEnableDering(bScalerWin , TRUE);
		//DLC//
		msDrvSetDLCEnable(bScalerWin, 1);
		//Histogram
		msDrvHistogramReportEnable(bScalerWin, 1);
	}

	//Delta mode for RGB input/output
	msACESetRGBMode(TRUE);
	msWrite2Byte( SC0B_B2, 0x000);
	msWrite2Byte( SC0B_B4, 0x000);

	//MWE
	{
        msWriteByte( SC00_5C, 0x08);
        msWrite2Byte( SC03_18, 0x100);
        msWrite2Byte( SC03_1A, 0x300);
        msWrite2Byte( SC03_1C, 0x300);
        msWrite2Byte( SC03_1E, 0x100);
        msWriteByte( SC03_02, 0x04);
		msWriteByte( SC03_05, 0x44);
		msWriteByte( SC03_07, 0x44);
		msWriteByte( SC03_09, 0x44);
		msWriteByte( SC03_0B, 0x44);
		msWrite2Byte( SC03_18, 0x2A7);
		msWrite2Byte( SC03_1A, 0x345);
		msWrite2Byte( SC03_1C, 0x576);
		msWrite2Byte( SC03_1E, 0x106);
		msWriteByte( SC65_2E, 0x1C);
		msWriteByte( SC65_49, 0xE0);
	}
	//noise masking
	msWriteByte( SC0B_24, 0x84);
	msWriteByte( SC0B_2C, 0x84);
	msWrite2Byte( SC0B_C0, 0x8484);
	msWrite2Byte( SC0B_C8, 0x8484);
	//CSC(SC07/SC22)
	msWriteByte( SC07_40, 0x11);
	msWriteByte( SC07_41, 0x33);
	msACESetIPR2YEnable(TRUE);
	msWrite2Byte( SC22_5A, 0x400);
	msWrite2Byte( SC22_5C, 0x000);
	msWrite2Byte( SC22_5E, 0x000);
	msWrite2Byte( SC22_60, 0x000);
	msWrite2Byte( SC22_62, 0x400);
	msWrite2Byte( SC22_64, 0x000);
	msWrite2Byte( SC22_66, 0x000);
	msWrite2Byte( SC22_68, 0x000);
	msWrite2Byte( SC22_6A, 0x400);

	//2p_DLC
	msWriteByte( SC5C_02, 0x0F);
	//FCC
	msWrite2Byte( SC07_80, 0x9B72);
	msWrite2Byte( SC07_82, 0x9B72);
	msWrite2Byte( SC07_90, 0xFFFF);
	msWrite2Byte( SC07_92, 0x7F7F);
	msWrite2Byte( SC07_9B, 0xFFFF);
	msWrite2Byte( SC07_9D, 0xFFFF);
}
...
void msACESetWinColorRange(BOOL bMain, E_WIN_COLOR_RANGE cr)
{
    ...
    {
        msSetPCColorMatrix(0);
        msSetPCColorMatrix(1);
    }
    else
    {
        msSetVideoColorMatrix(0);
        msSetVideoColorMatrix(1);
    }
...
{
    return _u8IsEnabledMWE;
}

void mdrv_ACE_MweEnable_Set(BOOL u8IsEnabledMEW)
{
    _u8IsEnabledMWE = u8IsEnabledMEW;
    if (TRUE == u8IsEnabledMEW)
    {
        msWriteByteMask(SC00_5C, BIT3, BIT3);
    }
    else
    {
        msWriteByteMask( SC00_5C, 0x00, BIT3 );
    }
