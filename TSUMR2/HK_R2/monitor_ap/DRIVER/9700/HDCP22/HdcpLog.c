...
// global
//----------------------------------------------------------------------//
static ENUM_HDCPLOG_PRIORITY g_enHdcp1xLogPriority = EN_HLOG_PRIORITY_SILENT;//[MT9700]EN_HLOG_PRIORITY_SILENT;//EN_HLOG_PRIORITY_INFO;//EN_HLOG_PRIORITY_INFO;
static ENUM_HDCPLOG_PRIORITY g_enHdcp2xLogPriority = EN_HLOG_PRIORITY_SILENT;//[MT9700]EN_HLOG_PRIORITY_SILENT;//EN_HLOG_PRIORITY_INFO;//EN_HLOG_PRIORITY_INFO;
...
void HDCP_PRINT(ULONG u32Priority, BYTE u8Version, const char* pu8Tag, const char* pu8Color, const char* pu8FnName, ULONG u32Line, const char* pu8Format, ...)
{
    ...
    vsnprintf(u8Msg, sizeof(u8Msg), pu8Format, args);//vsprintf(u8Msg, pu8Format, args);
    va_end(args);
...
void HDCP_PRINT_BYTE(ULONG u32Priority, BYTE u8Version, const char* pu8Format, ...)
{
    ...
    vsnprintf(u8Msg, sizeof(u8Msg), pu8Format, args);
    va_end(args);
...
void HDCPLOG_DumpHexValue(BYTE u8Version, unsigned char *pu8HexVal, unsigned int nLen)
{
    ...
    do
    {
        ...
        for (i=0; i<nLen; ++i)
        {
            ...
            snprintf((char *)u8Tmp,sizeof(u8Tmp), "%2X ", pu8HexVal[i]);
