...
enMsCEC_INT_EVENT _drv_CEC_SendFrame( BYTE header, enMsCEC_OPCODE_LIST opCode, BYTE* operand, BYTE length )
{
    ...
    msWriteByteMask(REG_CECALIVE_12_H, 0x0E, 0x0E);
    msWriteByteMask(REG_CECALIVE_12_H, 0x00, 0x0E);

    //Fill TX data
    msWriteByte(REG_CECALIVE_18_L, header);
    msWriteByte(REG_CECALIVE_18_H, opCode);
    ...
    if ( length > 0 )
    {
        ...
        for ( i=0; i < length; i++ )
            msWriteByte(REG_CECALIVE_19_L + i, *(operand + i));
    }

    if ((opCode = CEC_OPCODE_POLLING_MESSAGE) && (length == 0))
    {
        waitCnt = 5;
        msWriteByteMask(REG_CECALIVE_00_L, length, 0x0F); //set TX length
    }
    else
    {
        waitCnt = 30;
        msWriteByteMask(REG_CECALIVE_00_L, (length+1), 0x0F);
    }
    ...
    do
    {
        ...
        if (cnt++ >= waitCnt)
            break;
    } while ((msReadByte(REG_CECALIVE_11_H) & 0x0E) == 0);

    res = (enMsCEC_INT_EVENT)msReadByte(REG_CECALIVE_11_H);
...
void _drv_CEC_GetSelfAddress(void)
{
    ...
    msWriteByteMask(REG_CECALIVE_02_L, gStCECInfo.enMyLogicAddr << 4, 0xF0); //[7:4]: logical address
...
void drv_CEC_Init(void)
{
	...
	BYTE ucEfuseValue = msEread_GetDataFromEfuse(HDMI_EFUSE_ADDRESS_HRX_CEC);

    DRVCEC_PRINT_MSG("[CEC]func: drv_CEC_Init\r");

    _drv_CEC_InitVariable();

    msWriteByteMask(REG_CECALIVE_00_H, (0x10 | CEC_RETRY_CNT), 0x1F); //[4:3]: 1.5x low bit period [2:0]=CEC_RETRY_CNT
    msWriteByte(REG_CECALIVE_01_H, ((CEC_BUS_FREE_TIME << 4) | CEC_RESEND_INTERVAL)); //[3:0]: cnt1, [7:4]: cnt2
    msWriteByte(REG_CECALIVE_02_L,  ((CEC_LA_TV << 4) | CEC_FRAME_INTERVAL)); //[3:0]: cnt3, [7:4]: logical address: TV

    //Number of counts to achieve 10us
    clkCnt = (CRYSTAL_CLOCK % 100000L)*0.00016 + 0.5;
    msWriteByte(REG_CECALIVE_02_H, CRYSTAL_CLOCK/100000L); // integer part
    msWriteByteMask(REG_CECALIVE_03_L, clkCnt, 0x0F); // CEC time unit by Xtal(fractional)

    //select sw CEC ontroller
    msWriteByteMask(REG_CECALIVE_14_L, 0x00, _BIT1); //[1]: 0: clock source from xtal
    ...
    msWriteByte(REG_CECALIVE_01_L, 0x80);

    msWriteByteMask(REG_CECALIVE_03_H, 0x00, _BIT4); //[4]: select CEC controller, 0:software / 1: PM HW

    //clear CEC interrupt status
    msWriteByteMask(REG_CECALIVE_12_H, 0x1F, 0x1F);
    msWriteByteMask(REG_CECALIVE_12_H, 0x00, 0x1F);

    //undercheck
    //enable CEC function
    //msWriteByteMask(REG_PAD_TOP_03_L, BIT4, BIT4); //[4]: 1: Enable GPIO27 as CEC function

	//enable CEC function
	Init_hwCEC_Pin();

    #if (ENABLE_CEC_INT == 1)
    msWriteByteMask(REG_CECALIVE_13_L, 0x7E, 0x7F); //CEC Mask
    msWriteByteMask(REG_CECALIVE_30_L, 0, _BIT4); //enable IRQ
    msWriteByteMask(REG_000DA9, 0, _BIT0); // intr_ctrl for HK_R2
    #endif
	...
	if(ucEfuseValue &BIT7)
	{
		msWriteByteMask(REG_000499, (ucEfuseValue&(BIT6|BIT5))>>5, BIT1|BIT0);
	}
	else if(ucEfuseValue &BIT2)
	{
		msWriteByteMask(REG_0020CE, (ucEfuseValue&(BIT1|BIT0))<<5, BIT6|BIT5);
	}
...
void drv_CEC_CheckRxBuffer(void) //don't place print message in this function
{
    ...
    if ( msReadByte( REG_CECALIVE_11_H ) & _BIT0 ) //[0]: receive a new message successfully
    {
        gStCECInfo.stRxBufList[gStCECInfo.bMSGQEndIdx].bLength = (msReadByte(REG_CECALIVE_04_L) & 0x1F) + 1; //Get received message length
        ...
        if (gStCECInfo.stRxBufList[gStCECInfo.bMSGQEndIdx].bLength > 1)
        {
            for ( i = 0; i < gStCECInfo.stRxBufList[gStCECInfo.bMSGQEndIdx].bLength; i++ )
                gStCECInfo.stRxBufList[gStCECInfo.bMSGQEndIdx].bData[i] = msReadByte(REG_CECALIVE_20_L + i); //extract message data
        ...
        msWriteByteMask(REG_CECALIVE_12_H, _BIT0, _BIT0);
        msWriteByteMask(REG_CECALIVE_12_H, 0, _BIT0);
    }
}
...
{
    BOOL isMsgBoardcast = FALSE;
	BOOL isLenLegal = FALSE;
	BOOL retVal = FALSE;
...
void drv_CEC_ConfigWakeUp(void)
{
    ...
    msWriteByteMask(REG_CECALIVE_14_L, BIT1, BIT1); // CEC source clock select. 1: Internal OSC.

    //Number of counts to achieve 10us
    clkCnt = (CRYSTAL_CLOCK % 100000l)*0.00016 + 0.5;
    msWriteByte(REG_CECALIVE_02_H, CRYSTAL_CLOCK/1000001); // integer part
    msWriteByteMask(REG_CECALIVE_03_L, clkCnt, 0x0F); // CEC time unit by Xtal(fractional)
    ...
    msWriteByteMask(REG_CECALIVE_13_L, 0x1F, 0x1F); // Mask CEC interrupt in standby mode

    msWriteByteMask(REG_CECALIVE_01_L, 0x00, _BIT7); // [7]: Disable CEC controller
    msWriteByteMask(REG_CECALIVE_01_L, _BIT7, _BIT7); // [7]: Enable CEC controller

    msWriteByteMask(REG_CECALIVE_03_H, 0x50, 0x70); //[6:5]: Action for non-supported command, ,[4]: 0: software CEC controller; 1: power down hardware CEC controller;
    ...
    msWriteByte(REG_CECALIVE_07_L, 0x37); // Enable OP0~2 and OP4
    msWriteByte(REG_CECALIVE_07_H, 0x24); // Eanble OPCODE2's operand
    msWriteByte(REG_CECALIVE_08_L, 0x04); // OPCODE0: Image View On
    msWriteByte(REG_CECALIVE_08_H, 0x0D); // OPCODE1: Text View ON
    msWriteByte(REG_CECALIVE_09_L, 0x44); // OPCODE2: User control
    msWriteByte(REG_CECALIVE_0A_L, 0x82); // OPCODE4: Active source
    msWriteByte(REG_CECALIVE_0B_H, 0x40); // User control - Power
    msWriteByte(REG_CECALIVE_0C_L, 0x6D); // User control - Power ON Function
    msWriteByte(REG_CECALIVE_0D_H, 0x84); // [2:0]: CEC version 1.3a; [7:3]: OP4 is broadcast message
    ...
    msWriteByte(REG_CECALIVE_0F_L,0x00); // Device Vendor ID
    msWriteByte(REG_CECALIVE_0F_H,0x00); // Device Vendor ID
    msWriteByte(REG_CECALIVE_10_L,0x00); // Device Vendor ID
    msWriteByte(REG_CECALIVE_10_H,0x01); // [2:0]: Feature abort reason - "Not in correct mode to respond"

    //(7) Device Physical address: default is 0x00 0x00 0x00
    msWriteByte(REG_CECALIVE_0E_L, 0x00); // Physical address 0.0
    msWriteByte(REG_CECALIVE_0E_H, 0x00); // Physical address 0.0
    msWriteByte(REG_CECALIVE_14_H, 0x00); // Device type: TV

    //(8) Clear CEC status
    msWriteByte(REG_CECALIVE_11_L, 0x7F); // Clear CEC wakeup status
    msWriteByte(REG_CECALIVE_12_H, 0x1F); // Clear RX/TX/RF/LA/NACK status status
    msWriteByte(REG_CECALIVE_12_H, 0x00);

}

BOOL drv_CEC_GetInterruptFlag(void)
{
    return ((msReadByte(REG_000DB9) & BIT0) ?TRUE :FALSE);
}
