...
void mcuSetSlowClkSpeed(BYTE u8SlowClkDiv)
{
	BYTE    u8SpeedIdx;
    BYTE    u8Div;

 	u8SpeedIdx = (msReadByte(REG_0003B3) & 0x07);//clk select
    u8Div = (msReadByte(REG_000372) & 0x1F);//HKR2 clock divide counter

    if((u8SpeedIdx != IDX_MCU_R2_SLOW_CLK) || (u8Div != u8SlowClkDiv))//only for divide freq mode
    {
        msWriteByteMask(REG_000372, u8SlowClkDiv, 0x1F);
        msWriteByteMask(REG_0003BB, u8SlowClkDiv<<3, 0xF8);
	...
	else //set recovery clock and Timer
	{
	    mcuSetCPUClock(CLOCK_R2_SLOW_CLK, IDX_MCU_R2_SLOW_CLK);
	}
}
...
void mcuSetMcuSpeed( BYTE ucSpeedIdx )
{
    ...
    else if(ucSpeedIdx == IDX_MCU_R2_SLOW_CLK)
    {
        msWriteByteMask( REG_0003BC, 0x00, _BIT0);
        msWriteByteMask( REG_0003BB, 0x01, 0x07); //slow clock
        msWriteByteMask( REG_0003BC, _BIT0, _BIT0);
    }
    else
    {
    ...
    DRVMCU_printData("R2_CLK2 = %d MHz \r\n", (g_u32CpuClock/1000/1000));
