...
xdata const PropFontNonCompressType *pstPropFontSet1218;
...
{
    old_msWriteByte(OSD2_A4, HIBYTE(u16SN1) + (HIBYTE(u16SN2) <<4));
    old_msWriteByte(OSD2_A4, LOBYTE(u16SN1));
    old_msWriteByte(OSD2_A4, LOBYTE(u16SN2));
}
...
#if 1 // Use this in MST9U R2 due to data alignment issue (can't cast from WORD to DWORD)!
...
static void CopySubFontBuf(WORD* pu16Tar,const WORD*pu16Src,BYTE u8BitSrcLStart, BYTE u8BitWidth)
{   BYTE  i;
    WORD  u16Temp;
#if 0
    for(i=0; i < FONT_HEIGHT; i++)
...
{
    const PropFontNonCompressType *pstFontData = NULL;
    const PropFontNonCompressType *pstFontData1 = NULL;
    ...
    WORD  xdata tSN[FONT_HEIGHT]={0}; //May cause problem if use idata. Why???
    WORD  xdata tSN1[FONT_HEIGHT]={0};
...
#define CLR_LOAD_SINGLE_FONT1_FLAG()    (u8Flags&=~LOAD_SINGLE_FONT1_BIT)
    ...
    WRITE_FONT();
    #else
    _MEM_MSWRITE_BYTE(OSD2_A3, u8Addr);
    #endif
    ...
    while((u16StrIndex < u16FontCount) || (u16StrIndex1 < u16FontCount1))
    {
        ...
        //===================================================================================
        if((!LOAD_SINGLE_FONT1_FLAG) && (u16StrIndex1 < u16FontCount1) && (pu8Strings1 != NULL))   /// Handle 2nd strings
        {
    ...
    //old_msWriteByte(OSDIOA, OWEND_B|OSBM_B|ORBW_B);
    return u8FontCnt;
}
...
BYTE LoadShiftBmpFont(BYTE u8Addr, BYTE *u8FontTbl, WORD u16Size, BYTE u8UDShift)
{
    ...
    old_msWriteByte(OSD2_A3, u8Addr);
    i=0;
