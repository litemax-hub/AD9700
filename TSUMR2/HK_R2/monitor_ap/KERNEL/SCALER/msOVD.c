#define _MSOVD_G_
#include <math.h>
#include "types.h"
#include "board.h"
#include "Ms_rwreg.h"
#include "ms_reg.h"
#include "msFB.h"
#include "Mcu.h"
#include "Mode.h"
#include "UserPrefDef.h"
#include "Global.h"

//Internal
#include "msOVD.h"

typedef enum _ChipType
{
    CHIP_WARHOL,
    CHIP_RIBERA,
} ChipType;

typedef enum _ODRAMType
{
    OD_RAM_1M,//OD_MODE444_COMP
    OD_RAM_2M,//OD_MODE565
} ODRAMType;

ChipType  xdata msChipType = CHIP_WARHOL;
ODRAMType xdata msRAMType = OD_RAM_1M;

#define BIT0    0x0001
#define BIT1    0x0002
#define BIT2    0x0004
#define BIT3    0x0008
#define BIT4    0x0010
#define BIT5    0x0020
#define BIT6    0x0040
#define BIT7    0x0080


//#define OD_LIB_VER        0x03
//code ==================================================================

//*************************************************************************************************
//Function:  msODMemoryTest()
//Description: For Memory test
//
//           ex:    printData(" msODMemoryTest = %d"  , msODMemoryTest());
//           if the result is 0,
//           if the result is 1,
//
//*************************************************************************************************

//*************************************************************************************************
//Function:  msOD_Reset()
//Description: Reset MIU bank
//caller:        Power_TurnOnPanel of power.c
//
//           It need to reset MIU bank before turn on panel.
//*************************************************************************************************


//*************************************************************************************************
//Function:  msOD_OnOff()
//Description: Enable/Disable OverDrive and setup OD strength
//          ex:
//          msOD_OnOff( _OD_Disable, 0x50 ); //disable OD
//          msOD_OnOff( _OD_Enable, 0x5F ); //enable OD and set strength as 0x5F
//          OD strength: 0x50 (weak)  <------------>  0x5F (strong)
//
//*************************************************************************************************
//*************************************************************************************************
//Function:  msLoadODT()
//Description: OD look-up table loader.
//
//           ex: msLoadODT(tOverDrive)
//
//           It provides two kinds of format (normal and compressed) for OD look-up table.
//           The look-up table and OD lib should be located in the same bank!!
//
//*************************************************************************************************
#if ENABLE_RTE
BYTE code tOverDrive[] =
{
    ,
};

#if ENABLE_OD_MTG
BYTE code tOverDrive_MultiGain[] =
{
  0x3F,  0x3F,  0x3F,  0x3F,  0x3F,
  0x3F,  0x3F,  0x3F,  0x3F,  0x3F,
  0x3F,
};
#endif

#if ENABLE_OD_VRR
BYTE code tOverDrive_1[] =
{
    ,
};

BYTE code tOverDrive_2[] =
{
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,/*[0]  sram1 start*/
    ,/*[271]  sram1 end*/
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,/*[272]  sram2 start*/
    ,/*[543]  sram2 end*/
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,/*[544]  sram3 start*/
    ,/*[799]  sram3 end*/
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,/*[800]  sram4 start*/
    ,/*[1055]  sram4 end*/
};

BYTE code tAlphaTBL[] =
{
    ,

};
#endif


#ifdef _MULTI_OD_
BYTE code tMultiODTbl8_1[] =
{
    ,
};

code BYTE *tOverDrive_AllTbl[] =
{
   tOverDrive_120Y1, tOverDrive_120Y2, tOverDrive_120Y3, tOverDrive_120Y4,
   tOverDrive_120Y5, tOverDrive_120Y6, tOverDrive_120Y7, tOverDrive_120Y8,
   tOverDrive_110, tOverDrive_110, tOverDrive_110, tOverDrive_110,
   tOverDrive_110, tOverDrive_110, tOverDrive_110, tOverDrive_110,
   tOverDrive_100, tOverDrive_100, tOverDrive_100, tOverDrive_100,
   tOverDrive_100, tOverDrive_100, tOverDrive_100, tOverDrive_100
};



void msAPI_OverDriveMultiTable( BYTE index )
{
    idata WORD wCount,wXORKeyIdx;
    idata BYTE ucXORKey,OD_data,ucMultiTblIdx,ucSramIdx,ucTblData;
    idata DWORD dwXdataAddr;
    idata BYTE ucReg;
    idata BYTE ucOffset;

#if 0
    if(0 == tOverDrive_AllTbl) return;
#endif

    //mcuXdataMapToDRAM(MIU_OD_MTB_ADDR);
    //msWriteByte(SC16_00, 0x00);

    ucReg = msReadByte(SC16_20);
    msWriteBit(SC16_20, 0, _BIT0);  // Disable OD before load code
    while(msReadByte(SC16_20)&_BIT0);
    msWriteByte(SC16_02, 0x00);    // load code start

    dwXdataAddr = (WIN0_ADDR_START<<10);

    wCount=0;

    ucOffset = index*8;

    for(ucMultiTblIdx =0;ucMultiTblIdx < 8; ucMultiTblIdx++)
    {
        while(wCount<288)
        {
            for(ucSramIdx = 0;ucSramIdx < 4;ucSramIdx++)
            {
                if(ucSramIdx<3)
                    wXORKeyIdx =(9 + 10*ucSramIdx) + 272*(WORD)(ucSramIdx);
                else
                    wXORKeyIdx =(9 + 10*ucSramIdx) + 256*(WORD)(ucSramIdx-2) + 272*2;

                if( wCount == 9 && ucSramIdx == 0)
                    ucXORKey = 0;
                else if( wCount == 19 && ucSramIdx == 1)
                    ucXORKey = 0;
                else if( wCount == 29 && ucSramIdx == 2)
                    ucXORKey = 0;
                else if( wCount == 39 && ucSramIdx == 3)
                    ucXORKey = 0;
                else
                    ucXORKey =*(tOverDrive_AllTbl[ucMultiTblIdx+ucOffset]+wXORKeyIdx);

                if(ucSramIdx < 2 && wCount < 272)
                {
                    ucTblData = *(tOverDrive_AllTbl[ucMultiTblIdx+ucOffset]+(272*(WORD)ucSramIdx) + wCount ) ;
                    OD_data = ucXORKey ^ ucTblData;
                    msRegs[dwXdataAddr + ucSramIdx] = OD_data;
                }
                else if(ucSramIdx >=2 && wCount < 256)
                {
                    ucTblData = *(tOverDrive_AllTbl[ucMultiTblIdx+ucOffset]+(256*(WORD)(ucSramIdx-2) + 272*2) + wCount ) ;
                    OD_data = ucXORKey ^ ucTblData;
                    msRegs[dwXdataAddr + ucSramIdx] = OD_data;
                }
            }
            wCount++;
            dwXdataAddr += 0x10; //16 bytes alligment
        }
        wCount = 0;
    }

    msWriteBit(SC16_E1, _ENABLE, _BIT7);
    msWriteByteMask(SC16_E1, _BIT2 ,_BIT2 |_BIT1);

    msWriteByte(SC16_E3, 0x07);    // 8 multi-table

    msWriteByte(SC16_00, 0x01);
    msWriteBit(SC16_02, _ENABLE, _BIT1);
    msWrite2Byte(SC16_12, 288);     // sram size 288
    msWriteByte(SC16_18, 0x20);
    msWrite2Byte(SC16_08, (MIU_OD_MTB_ADDR+(WIN0_ADDR_START<<10))>>4);
    msWrite2Byte(SC16_0A, (MIU_OD_MTB_ADDR+(WIN0_ADDR_START<<10))>>20);

    msWriteByte(SC16_00, 0x00);
    msWriteBit(SC16_E4, _ENABLE, _BIT0);
    msWriteByte(SC16_E0, 91);    //#line start
    msWriteByte(SC16_20, ucReg);

    mcuXdataMapToDRAM(HK_XDATA_ADDR_START);
}
#endif

void msAPI_OverDriveEnable( BOOL bEnable )
{
   drvmsOverDriveOnOff(bEnable);
}
#if (CHIP_ID == CHIP_MT9701)
void msAPI_OverDriveTable(const BYTE *pODTbl, ODTBL_INDEX Tblindex)
{
    drvmsLoadODTable(pODTbl, Tblindex);
}
#else
void msAPI_OverDriveTable(void)
{
    drvmsLoadODTable(tOverDrive);
}
#endif

void msAPI_OverDriveAdjustWeighting(BYTE u8Weighting)
{
   drvmsOverDriveAdjustWeighting(u8Weighting);
}

void msAPI_OverDriveSetActiveWindow(BYTE enable, WORD u16HST,WORD u16HEND,  WORD u16VST, WORD u16VEND)
{
    drvmsOverDriveWindow(enable, u16HST,u16HEND,  u16VST, u16VEND);
}


void msAPI_OverDriveInit( void )
{
    drvmsOverDriverInit();

#if (CHIP_ID == CHIP_MT9701)
	msAPI_OverDriveTable(tOverDrive, OD_TABLE_0);
#else
	msAPI_OverDriveTable();
#endif

    msAPI_OverDriveAdjustWeighting(UserprefOverDriveWeighting);

    msAPI_OverDriveSetActiveWindow(_DISABLE, 0 , g_sPnlInfo.sPnlTiming.u16Width , 0 , g_sPnlInfo.sPnlTiming.u16Height);

#if ENABLE_OD_VRR
    drvmsOverDriverVRRInit();
    msAPI_OverDriveTable(tOverDrive_1, OD_TABLE_1); //Load OD_Table 1
    msAPI_OverDriveTable(tOverDrive_2, OD_TABLE_2); //Load OD_Table 2
    msAPI_OverDriveVRRAlphaTable_Load(tAlphaTBL); //Load OD_Alpha Table
#endif

#if ENABLE_OD_MTG
    msAPI_OverDriveMultiGain(0, 0, tOverDrive_MultiGain);
    msAPI_OverDriveMultiGainEnable(ENABLE);
#endif

}

#if ENABLE_OD_VRR
void msAPI_OverDriveVRRAlphaTable_Load(const BYTE *pAlphaTbl)
{
    drvmsLoadAlphaTable(pAlphaTbl);
}
void msAPI_OverDriveVRRForceTable_Enable(BOOL bEnable)
{
    drvmsOverDriveForceTableEnable(bEnable);
}
void msAPI_OverDriveVRRTable_Select(VRR_TBLSEC_INDEX Tblindex)
{
    drvmsOverDriveVRRTableSelect(Tblindex);
}
void msAPI_OverDriveVRRNode_Set(WORD u16VRRNode0, WORD u16VRRNode1, WORD u16VRRNode2)
{
    drvmsOverDriveVRRFrameRateNodeSet(u16VRRNode0, u16VRRNode1, u16VRRNode2);
}
WORD msAPI_OverDriveVRRFrameRate_Get(void)
{
    return drvmsOverDriveVRRFrameRateGet();
}

#if ENABLE_OD_AutoDownTBL
void msAPI_OverDriveTable2DRAM_Load(const BYTE *pODTbl, ODTBL_INDEX Tblindex, DWORD OD_ADL_ADDR_START)
{
    drvmsLoadODTable2DRAM(pODTbl, Tblindex, OD_ADL_ADDR_START);
}
void msAPI_OverDriveTableADL_Trig(DWORD OD_ADL_ADDR_START)
{
    drvmsTrigODADL(OD_ADL_ADDR_START);
}
void msAPI_OverDriveVRRAlphaTable2DRAM_Load(const BYTE *pAlphaTbl, DWORD OD_alpha_ADL_ADDR_START)
{
    drvmsLoadODAlphaTable2DRAM(pAlphaTbl, OD_alpha_ADL_ADDR_START);
}
void msAPI_OverDriveVRRAlphaTableADL_Trig(DWORD OD_alpha_ADL_ADDR_START)
{
    drvmsTrigODAlphaADL(OD_alpha_ADL_ADDR_START);
}
#endif

#endif

#if ENABLE_OD_MTG
///////////////////////////////////////////////////////////////////////////////
// <Name>: msAPI_OverDriveMultiGain
// <Description>: If enable OD multi gain, 
//                different OD gain will be applied to the different vertical position.
//                There are 11 OD gain section
//                => od_weight = OD multi gain enable ? gain_table value : user_weight
// <Parameter>:
//  u16VStart : The effect of OD multi gain starts after u16VStart (0~511)
//  u8MTGinterval : The interval of OD gain section 
//                  0 : 32 lines     2 : 128 lines 
//                  1 : 64 lines     3 : 256 lines
//                 
//  pODMTGTbl : OD multi gain table, you can ref tOverDrive_MultiGain[]
///////////////////////////////////////////////////////////////////////////////
void msAPI_OverDriveMultiGain(WORD u16Vstart, BYTE u8MTGinterval, BYTE *pODMTGTbl)
{
    msLoad_MultiODGain(u16Vstart, u8MTGinterval, pODMTGTbl);
}

void msAPI_OverDriveMultiGainEnable(Bool bEnable)
{
    mdrv_SC_MultiODGain_Enable(bEnable);
}

///////////////////////////////////////////////////////////////////////////////
// <Name>: msAPI_AdjustOverDriveMultiODGain
// <Description>: Adjust OD multi gain independently
//                
// <Parameter>:
//  u8MultiGainIdx : 0~10  
//  u8ODGain : OD multi gain (0x00~0xFF)
///////////////////////////////////////////////////////////////////////////////
void msAPI_AdjustOverDriveMultiODGain(BYTE u8MultiGainIdx, BYTE u8ODGain)
{
    mdrv_SC_MultiODGain_Adjust(u8MultiGainIdx, u8ODGain);
}
#endif

#else
BYTE code msOVDDummy[] = {0};
void msOverDriveDummy(void)
{
    BYTE x = msOVDDummy;
}

#endif
