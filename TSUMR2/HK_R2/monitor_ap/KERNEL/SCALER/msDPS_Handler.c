...
void msDPS_Handler ( void )
{
    ...
    xdata BYTE  i, j, Flag_ES60WhitePattern=0;
...
*/
    ...
    else
    {
        if ( Flag_BlendSchemeInPWM )
        {
	    	...
	    	if(LevelIndex > 9)
	        	LevelIndex = 9;
            currLumi = DPS_Lumi_Table[RowStart][LevelIndex];
            nextLevelIndex = LevelIndex + 1;
            if(nextLevelIndex > 10)
				nextLevelIndex = 10;
	    	nextLumi = DPS_Lumi_Table[RowStart][nextLevelIndex];
            TempVariable2 = 10UL * ( LumiMap - nextLumi );
            
            if (( currLumi - nextLumi) > 0)
            {
                TempVariable2 = TempVariable2/( currLumi - nextLumi); 
            }         
        ...
        else
        {
            ...
            if(LevelIndex > 15)
                LevelIndex = 15;
            currLumi = DPS_Lumi_Table[LevelIndex][10 - PowerLimit];
	    	nextLevelIndex = LevelIndex + 1;
            if(nextLevelIndex > 16)
                nextLevelIndex = 16;
	    	nextLumi = DPS_Lumi_Table[nextLevelIndex][10 - PowerLimit];
            TempVariable1 = 16UL * ( LumiMap - nextLumi);
            if (( currLumi - nextLumi) > 0)
            {
                TempVariable1 = TempVariable1 / ( currLumi - nextLumi);
            }
