...
void msSetPCColorMatrix()
{
    ...
    ucBank = old_msReadByte( BK0_00 ); // Backup bank
    ...
    old_msWriteByte( BK0_00, ucColorBank );
    ucColorBank = old_msReadByte( BK3_71 );
    old_msWriteByte( BK3_71, ucColorBank | BIT2 );
    msWriteColorMatrix( psOutTab );
    //old_msWriteByte( BK0_70, 0x15 ); // Enable color matrix&Change R/B range
    old_msWriteByte( BK3_71, ucColorBank );

    //old_msWriteByte( BK0_72, ucTmpBri );
    //old_msWriteByte( BK0_73, ucTmpBri );
    //old_msWriteByte( BK0_74, ucTmpBri );

    msDoubleBufferEnd();

    old_msWriteByte( BK0_00, ucBank ); // Restore bank
}
