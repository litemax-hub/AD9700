...
#define CECMSG_SUP_AUDIO_RETURN_CHANNEL         ENABLE_ARC //1.4
...
void api_CEC_ProcessCommand(void)
{
    ...
    CEC_PRINT_DATA("Receive CEC opcode = 0x%x\n", opcode);
    ...
    switch (opcode)
    {
        ...
        //----- Remote Control Passthrough ----------------
            ...
            case CEC_OPCODE_FEATURE_ABORT:
                if(drv_CEC_CheckFrame(CEC_MSGTRANS_DIRECT, 4))
                {
                    CEC_PRINT_DATA("Feature opcode = 0x%x\n", para0);
                    CEC_PRINT_DATA("Abort reason = 0x%x\n", para1);
                }
                break;
        #endif
            ...
            case CEC_OPCODE_SET_SYSTEM_AUDIO_MODE:
                if(drv_CEC_CheckFrame(CEC_MSGTRANS_BOTH, 3))
                {
                    if(para0 == 0) // OFF
                    {
                        #if ENABLE_ARC
            ...
            case CEC_OPCODE_INIT_ARC:
                if(drv_CEC_CheckFrame(CEC_MSGTRANS_DIRECT, 2))
                {
                    CEC_PRINT_MSG("CEC_OPCODE_INIT_ARC\n");

                    // May receive when AMP eARC ON -> OFF
                    if(sAudioArc.eStatus < AUDIO_ARC_STS_INIT)
                    {
                        msAPI_AudioArcInit(TRUE, AUDIO_ARC_STS_INIT);
                    }
                }
                break;
            ...
            case CEC_OPCODE_TERMINATE_ARC:
                if(drv_CEC_CheckFrame(CEC_MSGTRANS_DIRECT, 2))
                {
                    // May receive when AMP eARC OFF -> ON
                    CEC_PRINT_MSG("CEC_OPCODE_TERMINATE_ARC\n");
                    msAPI_AudioArcInit(FALSE, AUDIO_ARC_STS_OFF);
                }
                break;
...
#if(ENABLE_CEC_INT)
void api_CEC_ISRHandler(void)
{
    if(drv_CEC_GetInterruptFlag())
    {
        api_CEC_CheckRxBuffer();
    }
