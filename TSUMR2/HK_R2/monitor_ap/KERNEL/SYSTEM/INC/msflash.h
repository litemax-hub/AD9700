...
//#define TOTAL_BANK_NUM              ((g_FlashSize == FLASH_SIZE_4M)?(8):(4))// 4 // (TOTAL_BANK_NUM-1) : current code bank
//Customized bank for user data.
//Suggest to put after R2 code area, otherwise Target.ld should be modified for space reservation.
#define USER_DATA_BANK              (0?31:15)// customized user data space in flash, suppose R2 code < 15 banks
...
_MSFLASHDEC_ void FlashRestoreSpiMode( void );
//DOUBLE_WORD_ADDRESS_ACCESS
_MSFLASHDEC_ void FlashdwAddrPMCUnprotectEnable( DWORD dwAddr );
_MSFLASHDEC_ void FlashdwAddrDisableWP_GigaDevice_GD25M21B_GD25M41B(DWORD dwAddr);
_MSFLASHDEC_ void FlashdwAddrDisableWP( DWORD dwAddr );
_MSFLASHDEC_ void FlashdwAddrSectorErase( Bool bDoWP, DWORD dwAddr );
_MSFLASHDEC_ void FlashdwAddrWriteByte( Bool bDoWP, DWORD dwAddr, BYTE value );
_MSFLASHDEC_ DWORD FlashdwAddrRead4Byte( DWORD dwAddr);
_MSFLASHDEC_ BYTE FlashdwAddrReadByte( DWORD dwAddr );
_MSFLASHDEC_ BYTE FlashFSPdwAddrReadByte( DWORD dwAddr );
_MSFLASHDEC_ void FlashdwAddrWriteTbl( Bool bDoWP, DWORD dwAddr, BYTE *buffer, WORD count );
_MSFLASHDEC_ void FlashdwAddrMoveTbl( Bool bDoWP, DWORD dwSrcAddr, DWORD dwDestAddr, WORD count );
_MSFLASHDEC_ void FlashdwAddrReadTbl( DWORD dwAddr, BYTE *buffer, WORD count );
#endif
...
_MSFLASHDEC_ Bool FlashDetectSize(MS_U32 *u32FlashSize);
...
_MSFLASHDEC_ U32 FlashImageChksum(BOOL bAPOnly);
_MSFLASHDEC_ void FlashBinFileSizeCheck(void);
