...
void mbedtls_rsa_init( mbedtls_rsa_context *ctx,
               ...
               int hash_id )
{
    mbedtls_memset( ctx, 0, sizeof( mbedtls_rsa_context ) );
                      ...
                      size_t slen, mbedtls_md_context_t *md_ctx )
{
    ...
    mbedtls_memset( mask, 0, MBEDTLS_MD_MAX_SIZE );
    mbedtls_memset( counter, 0, 4 );
                               ...
                               unsigned char *sig )
{
    ...
    *p++ = 0;
    *p++ = MBEDTLS_RSA_SIGN;
    mbedtls_memset( p, 0xFF, nb_pad );
    p += nb_pad;
    *p++ = 0;

    if( md_alg == MBEDTLS_MD_NONE )
    {
        mbedtls_memcpy( p, hash, hashlen );
    }
    else
    {
        /*
    ...
    mbedtls_memcpy( sig, sig_try, ctx->len );
                               ...
                               const unsigned char *sig )
{
    ...
    if( mbedtls_memcmp( p + slen, result, hlen ) == 0 )
        return( 0 );
    else
                                 ...
                                 const unsigned char *sig )
{
    ...
    {
        if( mbedtls_memcmp( p, hash, hashlen ) == 0 )
            return( 0 );
        else
            return( MBEDTLS_ERR_RSA_VERIFY_FAILED );
    }
...
int mbedtls_rsa_self_test( int verbose )
{
    ...
    mbedtls_memcpy( rsa_plaintext, RSA_PT, PT_LEN );
    ...
    if( mbedtls_memcmp( rsa_decrypted, rsa_plaintext, len ) != 0 )
    {
        if( verbose != 0 )
        {
            mbedtls_printf( "failed\n" );
