...
{
    mbedtls_memset( ctx, 0, sizeof( mbedtls_ripemd160_context ) );
}
                        ...
                        const mbedtls_ripemd160_context *src )
{
    ...
    *dst = *src;
#else
    _m_memcpy(dst, src, sizeof(mbedtls_ripemd160_context));
#endif
}
                       ...
                       const unsigned char *input, size_t ilen )
{
    ...
    if( left && ilen >= fill )
    {
        mbedtls_memcpy( (void *) (ctx->buffer + left), input, fill );
    ...
    if( ilen > 0 )
    {
        mbedtls_memcpy( (void *) (ctx->buffer + left), input, ilen );
    }
}
...
int mbedtls_ripemd160_self_test( int verbose )
{
    ...
    mbedtls_memset( output, 0, sizeof output );
    ...
    for( i = 0; i < TESTS; i++ )
    {
        ...
        mbedtls_ripemd160( (const unsigned char *) ripemd160_test_input[i],
                   mbedtls_strlen( ripemd160_test_input[i] ),
                   output );

        if( mbedtls_memcmp( output, ripemd160_test_md[i], 20 ) != 0 )
        {
            if( verbose != 0 )
            {
