        ...
        else
        {
            ...
            else if(firstByteOfEntry == DIR_ENTRY_IS_DOT)
            {
                ...
                if((pstDirEntry->u8DirEntryAttrib & FAT_ATTRIB_DIRECTORY)==0
                    ||(!(_byteCompare((U8*)&strDOT[0],pstDirEntry->pu8DirEntryName,8)||_byteCompare((U8*)&strDOTDOT[0],pstDirEntry->pu8DirEntryName,8))))
                {
                    __ASSERT(0);
...
U8 FAT_GetNameByFileEntry(U8 enid, FileEntry *entry, U32 destMIUAddr,U8 charLen, BOOLEAN *bIsLongName)
{
    ...
    if((pFS_Env == NULL) || (!DirEntryIterator_Reset(pFS_Env->u8FileSystemID,currentClusterNo,currentDirEntryOffset,USB_IO_TEMP_ADDR)))
    {
        __ASSERT(0);
        return 0;
    }
...
static EN_ENTRY_CREATE_RESULT FAT_DirEntryFindEmpty( U8 u8EnvironmentID, U8 u8EntryCount )
{
    ...
    U32 u32FATTotalClusterNo;
    FS_EnvironmentStruct *pFS_Env = msAPI_FSEnv_GetEnvironment(u8EnvironmentID);
    ...
    while(TRUE)
    {
        ...
        /* get next cluster */
        if ( dirEntryIndexInCache >= dirEntryCacheCount && (dirEntryIndexInCache != 0))
        {
    ...
    while(TRUE)
    {
            ...
            (u8DIR_Attr==FAT_ATTRIB_VOLUME_ID))
        {
            ...
            for(j=0;j<(i+1);j++)
            {
                ...
                {
                    VolLabName[u8VolLabNameLenTemp] = ( ((U16)pu8DirEntryVolumeLabel[j] << 8) | pu8DirEntryVolumeLabel[j+1u]);
                    j++;
                }
                u8VolLabNameLenTemp++;
