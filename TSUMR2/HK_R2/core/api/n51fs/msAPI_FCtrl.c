...
EN_ENTRY_NEXT_RESULT msAPI_FCtrl_EntryNext(U8 enId, FileEntry* pFileToSet)
{
    ...
    memset(&tmpEntry, 0, sizeof(FileEntry));
    if(pFSEnv->u8FileSystemType==FILE_SYSTEM_TYPE_FAT)
    {
...
static EN_DRIVE_INITIAL_STATUS msAPI_FCtrl_MountDrive(U8 deviceIndex,EN_FILE_SYSTEM_TYPE fsType,U8* partitionBytes,U32 logicalBlockAddress,U8 partitionNo, U32 u32MBRLogicBlockAddress, U8 u8IndexInMBR)
{
    ...
    if(drvCount<DISK_DRIVE_NUM)
    {
        ...
        U8 u8Port = (U8)msAPI_MSDCtrl_GetDevicePort(deviceIndex);
 ...
 */
static unsigned int le32_to_U32(unsigned char *le32)
{
 ...
 */
static gpt_entry *msAPI_alloc_read_gpt_entries(U8 deviceIndex,gpt_header * pgpt_head)
{
    U32 count = 0;
    gpt_entry *pte = NULL;
    ...
    FCTRL_DBG(1, printf("%s: count = %u * %u = %u\n", __FUNCTION__,
    ...
    if (count != 0)
    {
        pte = (gpt_entry *)msAPI_FS_Memory_Allocate(512);
    }
    ...
    if(!msAPI_MSDCtrl_SectorRead(deviceIndex, (U32)le64_to_U64(pgpt_head->partition_entry_lba), 1, USB_IO_TEMP_ADDR))
    {
        FCTRL_DBG(1, printf("*** ERROR: Can't read GPT header ***\n"));
        msAPI_FS_Memory_Free(pte);
            ...
            gpt_header * pgpt_head)
{
    ...
    {
        FCTRL_DBG(1, printf("GUID Partition Table Header signature is wrong: 0x%llx != 0x%llx\n",
                              (unsigned long long)le64_to_U64(pgpt_head->signature), (unsigned long long)GPT_HEADER_SIGNATURE));

        FCTRL_DBG(1, printf("pgpt_head->revision: 0x%llx \n",(unsigned long long)le64_to_U64(pgpt_head->revision) ));
        FCTRL_DBG(1, printf("pgpt_head->header_size: 0x%x\n",pgpt_head->header_size ));
    ...
    if(le64_to_U64(pgpt_head->my_lba) != lba)
    {
        FCTRL_DBG(1, printf("GPT: my_lba incorrect: %llx != %llx\n",
                              (unsigned long long)le64_to_U64(pgpt_head->my_lba), (unsigned long long)lba));
        return 0;
    }

    /* Check the first_usable_lba and last_usable_lba are within the disk. */
    //last_lba = (unsigned long long) msAPI_MSDCtrl_GetBlockNum(deviceIndex);
    last_lba = le64_to_U64(pgpt_head->last_usable_lba);
...
static int msAPI_is_pte_valid(gpt_entry * pte)
{
    ...
    {
        FCTRL_DBG(1, printf("%s: Found an unused PTE GUID at %p\n", __FUNCTION__, pte));
        return 0;
    }
    else
    {
...
U8 msAPI_FCtrl_LoadGPT(U8 deviceIndex, void * mbr, U32 *PartitionStatus)
{
    ...
    if (msAPI_is_gpt_valid(deviceIndex, pgpt_pte, (unsigned long long)GPT_PRIMARY_PARTITION_TABLE_LBA, &(gpt_head)) != 1)
    {
    ...
    for (i = 0; i < (U32)le32_to_U32(gpt_head.num_partition_entries); i++)
    {
        ...
        {
            /* Read GPT Entries from device */
            if(!msAPI_MSDCtrl_SectorRead(deviceIndex, (U32)le64_to_U64(gpt_head.partition_entry_lba) + block_count, 1, USB_IO_TEMP_ADDR))
            {
                FCTRL_DBG(1, printf("GPT: Failed to allocate memory for PTE no:%d\n", block_count*4));
                msAPI_FS_Memory_Free(*pgpt_pte);
        ...
        if (msAPI_is_pte_valid(&(*pgpt_pte)[entry_no]))
        {
            ...
            if(initialStatus==DRIVE_INITIAL_STATUS_ERROR)
            {
                FCTRL_DBG(1, printf("   Initial Partition Failed on DeviceID %u at Partition %u\n\n",deviceIndex,GPTpartitionNo));
                continue;
            }
            else if(initialStatus==DRIVE_INITIAL_STATUS_SUCCESS)
            {
                FCTRL_DBG(1, printf("    Initial Partition Success on DeviceID %u at Partition %u\n\n",deviceIndex,GPTpartitionNo));
                FS_SETBIT(*PartitionStatus,GPTpartitionNo);
            }
            else if(initialStatus==DRIVE_INITIAL_STATUS_MOUNTED_ALREADY)
            {
                FCTRL_DBG(1, printf("    Partition %u on DeviceID %u Already Initialized\n\n",GPTpartitionNo,deviceIndex));
                FS_SETBIT(*PartitionStatus,GPTpartitionNo);
            }
...
U32 msAPI_FCtrl_LoadPartitionsOnDevice(U8 deviceIndex)
{
    ...
    if(!partitionNo)
    {
        ...
        for(i=0;i<4;i++)//primary
        {
            ...
            U32 u32LogicBlockAddress=0, u32LogicBlockAddress1=0;      //LBA
            U32 u32LengthOfPartition=0, u32LengthOfPartition1=0;
        ...
        *pbIsUnicode = FALSE;
        ...
        if(!bIsNoName)
        {
            ...
            for(j=0;j<(i+1);j++)
            {
                ...
                {
                    VolLabName[u8VolLabNameLenTemp] = (U16)( ((U16)pu8TempVolLab[j] << 8) | pu8TempVolLab[j+1u]);
                    j++;
                }
                u8VolLabNameLenTemp++;
...
static U8 msAPI_FCtrl_GetNameByFileEntry(U8 enId, FileEntry* pEntry, U32 MIUAddr, U8 charLen, BOOLEAN *bIsLongName)
{
    U8 fsType;
    U8 len=0;
    FS_EnvironmentStruct* pFSEnv=msAPI_FSEnv_GetEnvironment(enId);
    FileEntry tmpEntry;
...
U8 msAPI_FCtrl_FileCreate_WithoutNameChecking(U8 u8enid, U16 *pu16LongName, U8 charLen)
{
    U8 u8Result;
    U8 fsType;
    FS_EnvironmentStruct* pFSEnv=msAPI_FSEnv_GetEnvironment(u8enid);
...
{
    BOOLEAN bRet = FALSE;
    MSDCtrl_Drive target_drive = {0, 0,  FILE_SYSTEM_TYPE_NONE, 0, 0, 0, 0};
