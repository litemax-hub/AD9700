...
#if ENABLE_MSTV_UART_DEBUG && MAPI_MBX_DEBUG
#define MAPI_MBX_PRINT(format, ...)     printf(format, ##__VA_ARGS__)
#else
#define MAPI_MBX_PRINT(format, ...)
#endif
////////////////////////////////////////////////////////////////////////////////
...
//-------------------------------------------------------------------------------------------------
MBX_Result MApi_MBX_SetInformation(MS_U8 *pU8Info, MS_U8 u8Size)
{
    return MDrv_MBX_SetInformation(E_MBX_ROLE_CP, pU8Info, u8Size);
}

/// House Keeping do not support this API.
MBX_Result MApi_MBX_GetInformation(MS_U8 *pU8Info, MS_U8 u8Size)
{
...
void MBX_Handler(void)
{
    ...
    if(MApi_MBX_RecvMsg(E_MBX_CLASS_SAR, &MB_Message, 0, MBX_CHECK_NORMAL_MSG) == E_MBX_SUCCESS)
    {
        if(MB_Message.u8Index == MB_SAR_CMD_READ_DRAM)
        {
            for(i=0; i<MB_Message.u8ParameterCount; i++)
            {
                addr += MB_Message.u8Parameters[i];
                if(i != (MB_Message.u8ParameterCount)-1)
                    addr <<= 8;
            }
            u8MailButton = msMemReadByte(addr);
        }
    }
#endif
}
